...
import { serializeExportManifestToCsv } from "../Backend/kai/services/kaiExportManifestCsvSerializer.js";

const basePath = "/api/kai/sprint2/intake";
const routePath = "/admin/organizations/:organizationId/export-manifests/:exportManifestId/csv";
...
const csv = "citation_ref,claim_id,evidence_item_id,source_id,source_version_id,limitation_codes\r\n";
...
function downloadPath(overrides = {}) {
  return `${basePath}/admin/organizations/${overrides.organizationId || organizationId}`
    + `/export-manifests/${overrides.exportManifestId || exportManifestId}/csv`
    + (overrides.query || "");
}
...
function createScenario(overrides = {}) {
  return {
    ...
    actorContext,
    serviceCalls: [],
    serviceResult: { ok: true, data: { csvContractVersion: "kai-sprint2-export-manifest-csv-v1", csv }, error: null },
    ...overrides,
  };
}
...
test("authorized CSV export delivery route appears as exactly one authenticated GET route", () => {
...
test("authorized human request returns the existing deterministic CSV as an attachment", async (t) => {
  ...
  const restoreService = intakeRouteTestables.setIntakeServiceForTest({
    async serializeExportManifestToCsv(input) {
  ...
  assert.equal(response.headers["content-disposition"], 'attachment; filename="kai-export-manifest-evidence-appendix.csv"');
  assert.equal(response.bodyText, csv);
...
test("client-controlled filename or path material cannot enter the attachment filename", async (t) => {
  ...
  const response = await requestDownload(server, downloadPath({
    query: "?filename=..%2F..%2Fsecret.csv&original_filename=client.md",
  }));

  assert.equal(response.statusCode, 200);
  const disposition = response.headers["content-disposition"];
  assert.equal(disposition, 'attachment; filename="kai-export-manifest-evidence-appendix.csv"');
...
test("upstream failures propagate through the existing KAI API error convention with no attachment", async (t) => {
  ...
  for (const [code, statusCode] of [
    ...
    assert.notEqual(response.headers["content-type"], "text/csv; charset=utf-8", code);
  }
});

test("authentication failure and malformed identifiers do not call the CSV service", async (t) => {
...
test("AI/system actors are denied by the existing governed CSV wrapper before rendering", async () => {
  let renderCalls = 0;
...
test("route helper sends no attachment on malformed service success", () => {
  ...
  intakeRouteTestables.sendCsvAttachment(res, { ok: true, data: { csvContractVersion: "v1" }, error: null });
...
test("route and package sources contain no SQL, direct KAI DB-helper access, artifact persistence, or storage download framework", () => {
  ...
  const routeStart = routeSource.indexOf('"/admin/organizations/:organizationId/export-manifests/:exportManifestId/csv"');
  ...
  assert.match(routeSlice, /serializeExportManifestToCsv/);
  ...
  const allSources = [
    routeSource,
    readFileSync("Backend/kai/services/kaiExportManifestCsvSerializer.js", "utf8"),
