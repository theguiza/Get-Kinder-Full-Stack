...
import { serializeExportManifestToDocx } from "../Backend/kai/services/kaiExportManifestDocxSerializer.js";

const basePath = "/api/kai/sprint2/intake";
const routePath = "/admin/organizations/:organizationId/export-manifests/:exportManifestId/docx";
...
const docxBytes = Buffer.from("PK\x03\x04-stub-docx-body", "latin1");
...
function downloadPath(overrides = {}) {
  return `${basePath}/admin/organizations/${overrides.organizationId || organizationId}`
    + `/export-manifests/${overrides.exportManifestId || exportManifestId}/docx`
    + (overrides.query || "");
}
...
function createScenario(overrides = {}) {
  return {
    ...
    actorContext,
    serviceCalls: [],
    serviceResult: { ok: true, data: { docxContractVersion: "kai-sprint2-export-manifest-docx-v1", docx: docxBytes }, error: null },
    ...overrides,
  };
}
...
test("authorized DOCX export delivery route appears as exactly one authenticated GET route", () => {
...
test("authorized human request returns the exact governed DOCX bytes as an attachment", async (t) => {
  ...
  const restoreService = intakeRouteTestables.setIntakeServiceForTest({
    async serializeExportManifestToDocx(input) {
  ...
  assert.equal(response.headers["content-disposition"], 'attachment; filename="kai-export-manifest.docx"');
  assert.equal(response.bodyBuffer.equals(docxBytes), true);
...
test("client-controlled filename or path material cannot enter the attachment filename", async (t) => {
  ...
  const response = await requestDownload(server, downloadPath({
    query: "?filename=..%2F..%2Fsecret.docx&original_filename=client.docx",
  }));

  assert.equal(response.statusCode, 200);
  const disposition = response.headers["content-disposition"];
  assert.equal(disposition, 'attachment; filename="kai-export-manifest.docx"');
...
test("authentication failure and malformed identifiers do not call the DOCX service", async (t) => {
...
test("AI/system actors are denied by the existing governed DOCX wrapper before rendering", async () => {
  let renderCalls = 0;
...
test("route helper sends no attachment on malformed service success", () => {
  ...
  intakeRouteTestables.sendDocxAttachment(res, { ok: true, data: { docxContractVersion: "v1" }, error: null });
...
test("route and package sources contain no SQL, direct KAI DB-helper access, artifact persistence, or storage download framework", () => {
  ...
  const routeStart = routeSource.indexOf('"/admin/organizations/:organizationId/export-manifests/:exportManifestId/docx"');
  ...
  assert.match(routeSlice, /serializeExportManifestToDocx/);
  ...
  const allSources = [
    routeSource,
    readFileSync("Backend/kai/services/kaiExportManifestDocxSerializer.js", "utf8"),
