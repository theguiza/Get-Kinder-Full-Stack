...
const injectedStartDto = Object.freeze({
...
function concretePath(overrides = {}) {
  return `${basePath}/admin/organizations/${overrides.organizationId || organizationId}`
    ...
    + `/export-review-queue/${overrides.exportReviewQueueItemId || exportReviewQueueItemId}/start`;
}
...
function createScenario(overrides = {}) {
  return {
    ...
    actorContext,
    serviceCalls: [],
    serviceResult: { ok: true, data: injectedStartDto, error: null },
    events: [],
    ...overrides,
  };
}
...
test("P3-10 route appears as exactly one mounted authenticated POST route", () => {
...
test("P3-10 export-review start route", async (t) => {
  ...
  await t.test("exact path/body values and middleware actorContext are forwarded once", async () => {
    ...
    assert.deepEqual(response.body, { ok: true, data: injectedStartDto, warnings: [] });
...
test("P3-10 route source imports no database or repository layer and performs no writes", () => {
  ...
  assert.match(source, /function validateStartExportReviewRequestOrSend/);
  assert.match(slice, /startGeneratedDraftExportReview/);
