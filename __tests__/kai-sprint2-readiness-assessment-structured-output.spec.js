// Readiness Assessment generator-result contract repair, sibling to the
// Impact Narrative structured-output repair.
//
...
// fails closed via the existing reason tags, that a genuinely
...
test("readiness assessment structured-output: the provider request carries the exact bounded output_config.format json_schema", async () => {
  const { generator, calls } = generatorWithResponse({
    content: [{ type: "text", text: JSON.stringify({ blocks: [{ text: "A readiness gap remains for claim traceability.", citations: [{ claimId: CLAIM, evidenceItemId: EVIDENCE }] }] }) }],
  });
  await generator({ contentType: "readiness_assessment", requestedAudience: "internal", readiness: READINESS, claims: GOOD_CLAIMS });

  assert.equal(calls.length, 1);
  assert.deepEqual(calls[0].output_config, {
    format: {
      type: "json_schema",
      schema: __readinessAssessmentDraftGeneratorContract.READINESS_ASSESSMENT_OUTPUT_SCHEMA,
    },
  });
...
test("readiness assessment structured-output: a valid schema-conformant response normalizes into the existing { blocks: [...] } shape and passes classifyGeneratorResult", async () => {
  const { generator } = generatorWithResponse({
    content: [{ type: "text", text: JSON.stringify({ blocks: [{ text: "A readiness gap remains for claim traceability.", citations: [{ claimId: CLAIM, evidenceItemId: EVIDENCE }] }] }) }],
  });
  const result = await generator({ contentType: "readiness_assessment", requestedAudience: "internal", readiness: READINESS, claims: GOOD_CLAIMS });

  assert.deepEqual(result, {
    blocks: [{ ordinal: 1, text: "A readiness gap remains for claim traceability.", citations: [{ claimId: CLAIM, evidenceItemId: EVIDENCE }] }],
  });
  assert.equal(validateGeneratorResult(result), true);
  assert.deepEqual(classifyGeneratorResult(result), { ok: true, reason: null });
});

test("readiness assessment structured-output: invalid generator input (missing readiness.requirements) is rejected as generator_result_input_contract_rejected before any provider call", async () => {
  const { generator, calls } = generatorWithResponse({ content: [{ type: "text", text: JSON.stringify({ blocks: [] }) }] });
  const result = await generator({ contentType: "readiness_assessment", requestedAudience: "internal", claims: GOOD_CLAIMS });
...
test("readiness assessment structured-output: no extractable provider text fails closed as generator_result_provider_text_missing", async () => {
...
test("readiness assessment structured-output: unparseable provider text fails closed as generator_result_json_parse_failed", async () => {
...
test("readiness assessment structured-output: a non-object JSON root fails closed as generator_result_json_root_invalid", async () => {
...
test("readiness assessment structured-output: a missing/non-array blocks field fails closed as generator_result_blocks_field_invalid", async () => {
...
test("readiness assessment structured-output: a genuinely empty blocks array from a schema-conformant response still classifies as generator_result_blocks_empty", async () => {
...
test("readiness assessment structured-output: malformed citations still fail closed under the existing shared classifier", async () => {
  const { generator } = generatorWithResponse({
    content: [{ type: "text", text: JSON.stringify({ blocks: [{ text: "A readiness gap remains.", citations: [{ claimId: "not-a-uuid", evidenceItemId: EVIDENCE }] }] }) }],
  });
  const result = await generator({ contentType: "readiness_assessment", requestedAudience: "internal", readiness: READINESS, claims: GOOD_CLAIMS });
  assert.equal(validateGeneratorResult(result), false);
...
test("readiness assessment structured-output: exactly one provider call occurs per generation, success or failure", async () => {
  for (const response of [
    { content: [{ type: "text", text: JSON.stringify({ blocks: [{ text: "A readiness gap remains.", citations: [{ claimId: CLAIM, evidenceItemId: EVIDENCE }] }] }) }] },
    { content: [] },
...
test("readiness assessment structured-output: unparseable raw provider text never leaks into the fail-closed result or its reason", async () => {
  const rawText = "SENTINEL-RAW-PROVIDER-TEXT-0003 not json";
