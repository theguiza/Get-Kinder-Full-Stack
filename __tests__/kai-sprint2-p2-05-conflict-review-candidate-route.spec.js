...
/**
 * P2-05 potential conflict-review candidate composition proof. Mirrors the
 * existing proven `kai-sprint2-p2-04-claim-gap-followup-route.spec.js`
 * pattern exactly. P2-05's own claim-pair-normalization/lineage/persistence/
 * replay logic is not retested here - it remains owned by
 * `kai-sprint2-p2-05-conflict-review-candidate-boundary.spec.js`,
 * `kai-sprint2-p2-05-conflict-review-candidate.integration.spec.js`, and the
 * P2-05 verifier script. This file proves only the new mounted boundary: the
 * route forwards server-resolved organizationId/firstClaimId/secondClaimId/
...
const routePath = "/admin/organizations/:organizationId/claims/:firstClaimId/potential-conflicts/:secondClaimId";
const organizationId = "00000000-0000-4000-8000-000000000001";
const firstClaimId = "a0000000-0000-4000-8000-000000000001";
const secondClaimId = "a0000000-0000-4000-8000-000000000002";
...
const injectedConflictCandidateDto = Object.freeze({
  conflictGroup: {
    conflict_group_id: "b0000000-0000-4000-8000-000000000001",
    lower_claim_id: firstClaimId,
    higher_claim_id: secondClaimId,
  },
...
function concretePath(overrides = {}) {
  return `${basePath}/admin/organizations/${overrides.organizationId || organizationId}`
    + `/claims/${overrides.firstClaimId || firstClaimId}/potential-conflicts/${overrides.secondClaimId || secondClaimId}`;
}
...
function createScenario(overrides = {}) {
  return {
    ...
    serviceCalls: [],
    dependencyCalls: [],
    serviceResult: { ok: true, data: injectedConflictCandidateDto, error: null },
    events: [],
    ...overrides,
  };
}
...
test("P2-05 potential-conflict-review-candidate route appears as exactly one mounted authenticated POST route", () => {
...
test("P2-05 potential-conflict-review-candidate route", async (t) => {
  ...
  const restore = intakeRouteTestables.setIntakeServiceForTest({
    async createConflictReviewCandidate(input, dependencies) {
  ...
  await t.test("unauthenticated requests fail before the P2-05 write seam", async () => {
  ...
  await t.test(
    "an authenticated mapped human in the correct organization reaches the service exactly once with the exact requested claim ids, server-resolved identity, and a production metadataOnlyAudit dependency",
    async () => {
      ...
      assert.deepEqual(response.body, { ok: true, data: injectedConflictCandidateDto, warnings: [] });
      ...
      const rejectedNoGroupId = dependencies.metadataOnlyAudit.prepareMetadataOnlyAudit({
        payload: { attempted_operation: "conflict_review_candidate_created", object_type: "conflict_group" },
      });
      assert.equal(rejectedNoGroupId.ok, false, "the production adapter must refuse a payload with no conflict_group_id");
      const prepared = dependencies.metadataOnlyAudit.prepareMetadataOnlyAudit({
        payload: {
          ...
          conflict_group_id: injectedConflictCandidateDto.conflictGroup.conflict_group_id,
        },
      });
  ...
  await t.test("the route never reinterprets or normalizes claim order: firstClaimId/secondClaimId reach the service exactly as requested, even when second < first", async () => {
    scenario = createScenario();
    const response = await requestJson(server, concretePath({ firstClaimId: secondClaimId, secondClaimId: firstClaimId }));

    assert.equal(response.statusCode, 200);
    assert.equal(scenario.serviceCalls.length, 1);
    assert.equal(scenario.serviceCalls[0].firstClaimId, secondClaimId);
    assert.equal(scenario.serviceCalls[0].secondClaimId, firstClaimId);
  });

  await t.test("request body cannot inject actor/time/conflict-group fields", async () => {
    scenario = createScenario();
    const response = await requestJson(
      ...
      `${concretePath()}?organization_id=00000000-0000-4000-8000-000000000999&actorContext=%7B%22actorType%22%3A%22system%22%7D`,
      {
        body: {
          ...
          conflict_group_id: "ffffffff-0000-4000-8000-000000000001",
  ...
  await t.test("malformed path values use the existing safe validation_blocker response", async () => {
    for (const path of [
      ...
      concretePath({ firstClaimId: "not-a-uuid" }),
      concretePath({ secondClaimId: "not-a-uuid" }),
      concretePath({ organizationId: "a0000000-0000-4000-8000-000000000001".toUpperCase() }),
      concretePath({ firstClaimId: firstClaimId.toUpperCase() }),
      concretePath({ secondClaimId: secondClaimId.toUpperCase() }),
    ]) {
      scenario = createScenario();
...
test("P2-05 potential-conflict-review-candidate route: KAI_SPRINT2_ENABLED=false produces zero service/write activity", async (t) => {
  const originalFeatureFlag = process.env.KAI_SPRINT2_ENABLED;
...
test("P2-05 potential-conflict-review-candidate route source imports no database or repository layer, performs no writes, and invokes no P2-04 or P2-06+ service", () => {
  ...
  assert.match(source, /async function getConflictReviewCandidateService/);
  assert.match(slice, /createConflictReviewCandidate/);
  ...
  assert.doesNotMatch(slice, /p2-04|p2-06|p2-07|p2-08|kaiClaimGapFollowupService|kaiExportCandidateService/i);
});

test("P2-05 potential-conflict-review-candidate route source never contains the literal token req.user", () => {
  const source = readFileSync("Backend/kai/routes/sprint2IntakeApi.js", "utf8");
