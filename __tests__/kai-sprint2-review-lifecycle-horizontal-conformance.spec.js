...
test("section 7 - the six lifecycle-facing allowlists all include exactly the same six current content types", () => {
  ...
  assert.match(exportReviewServiceSource, /const ALLOWED_GENERATED_CONTENT_TYPES = new Set\(\[\s*"evidence_summary",\s*"impact_narrative",\s*"readiness_assessment",\s*"data_gap_memo",\s*"case_for_support",\s*"board_update",?\s*\]\)/);
  ...
  assert.match(libraryServiceSource, /const LIBRARY_CONTENT_TYPES = new Set\(\[\s*"evidence_summary",\s*"impact_narrative",\s*"readiness_assessment",\s*"data_gap_memo",\s*"case_for_support",\s*"board_update",?\s*\]\)/);

  const readModelSource = readFileSync(new URL("../Backend/kai/db/kaiGeneratedDraftLibraryReadModels.js", import.meta.url), "utf8");
  assert.match(readModelSource, /d\.content_type IN \('evidence_summary', 'impact_narrative', 'readiness_assessment', 'data_gap_memo', 'case_for_support', 'board_update'\)/);
});
...
test("horizontal review-lifecycle conformance: all six content types were exercised", () => {
