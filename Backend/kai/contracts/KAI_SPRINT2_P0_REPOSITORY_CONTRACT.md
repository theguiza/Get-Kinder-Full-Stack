...
The internal repository interface defines one exact-identity, repository-neutral signal for an idempotent repository write conflict. Only batch creation is integrated with that signal in this leaf. No live SQL adapter is claimed to emit it. PostgreSQL mapping, constraints, two-session proof, and atomicity remain Gate-A-dependent, and deployed-schema compatibility remains `NOT_CONFIRMED`.
