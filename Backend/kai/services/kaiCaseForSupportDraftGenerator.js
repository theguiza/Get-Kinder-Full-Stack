...
const CONTENT_TYPE = "case_for_support";
const REQUESTED_AUDIENCE = "internal";
const FUNDER_REQUESTED_AUDIENCE = "funder";
// P13-EXT-1: case_for_support generation is internal + funder only - public
// is never accepted here (rejected earlier, at the service-level input
// validator, isCreateCaseForSupportDraftInput in
// kaiGeneratedContentService.js). Mirrors
// kaiEvidenceSummaryDraftGenerator.js's ALLOWED_REQUESTED_AUDIENCES shape.
...
const CASE_FOR_SUPPORT_OUTPUT_SCHEMA = Object.freeze({
  type: "object",
  required: ["blocks"],
...
export function createProductionCaseForSupportDraftGenerator({
    ...
    const response = await createMessage({
      ...
      system: [
        generatorInput.requestedAudience === FUNDER_REQUESTED_AUDIENCE
          ? "You generate funder-facing Case for Support narratives for Get Kinder."
          : "You generate internal Case for Support narratives for Get Kinder.",
        "A Case for Support explains need, work, outcomes/impact, limitations, and the rationale for support - it is a standalone evidence-backed narrative.",
        ...
        "Never invent a funding ask, a dollar amount, a donor strategy, a beneficiary story, an organizational claim, or a needs statement.",
        "Every statement of need, impact, or rationale for support must be supported by and traceable to a cited claim -- if the supplied claims do not state a need, an ask, a dollar figure, or a beneficiary story, do not write one.",
      ...
      output_config: {
        format: {
          type: "json_schema",
          schema: CASE_FOR_SUPPORT_OUTPUT_SCHEMA,
        },
      },
    });
...
export const __caseForSupportDraftGeneratorContract = Object.freeze({
  CONTENT_TYPE,
  REQUESTED_AUDIENCE,
  FUNDER_REQUESTED_AUDIENCE,
  ALLOWED_REQUESTED_AUDIENCES,
  MODEL,
  CASE_FOR_SUPPORT_OUTPUT_SCHEMA,
});
