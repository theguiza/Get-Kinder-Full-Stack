...
// Reuses the exact existing export-review authority (gk_admin only) for the
// COMPLETE transition too - never a second review authority for packets.
const COMPLETE_GRANT_RESPONSE_PACKET_EXPORT_REVIEW_ROLES = new Set(EXPORT_CANDIDATE_ALLOWED_ROLES);
const COMPLETE_GRANT_RESPONSE_PACKET_EXPORT_REVIEW_OPERATION = "complete_grant_response_packet_export_review";
...
// expectedUpdatedAt + actorContext + now), and NOTHING else - no members,
// generatedContentDraftIds, member ordering, canonicalFingerprint,
// memberCount, packet audience, manifest identity, or approval/final-
// release decision is ever accepted from a caller.
...
// 'export_review' queue row from in_progress/needs_gk_review to
// resolved/resolved. Completion means only that a gk_admin completed the
// governed human export review of this exact immutable packet candidate -
// it grants NO final eligibility evaluation, NO approval, NO funder/public
// readiness, NO export authority, NO final release, and NO manifest.
// Reuses the one existing governed 'export_review' queue_type/lifecycle -
// it does not create a parallel packet review state machine.
export async function completeGrantResponsePacketExportReview(input, dependencies = {}) {
  const env = dependencies.env || process.env;
  if (!isKaiSprint2Enabled(env)) return buildKaiError("feature_disabled", { data: null });
  if (!isKaiGenerationEnabled(env)) return buildKaiError("feature_disabled", { data: null });
  if (!isCompleteGrantResponsePacketExportReviewInput(input)) return buildKaiError("validation_blocker", { data: null });
  if (!isMappedHumanActor(input.actorContext)) return buildKaiError("authorization_denied", { data: null });

  const auth = validateActorCanPerformOperation(
    input.actorContext,
    COMPLETE_GRANT_RESPONSE_PACKET_EXPORT_REVIEW_OPERATION,
    input.organizationId,
    { allowedRoles: COMPLETE_GRANT_RESPONSE_PACKET_EXPORT_REVIEW_ROLES },
  );
  if (!auth.ok) {
    return buildKaiError(auth.error_code || "authorization_denied", { blockers: auth.blockers, data: null });
  }
  ...
  const repository =
    dependencies.grantResponsePacketExportCandidateRepository
    || (await createDefaultGrantResponsePacketExportCandidateRepository());
  const result = await repository.completeGrantResponsePacketExportReview(input, {
    metadataOnlyAudit: dependencies.metadataOnlyAudit,
  });
  if (!result.ok) return buildKaiError(result.error.code, { status: result.error.status, data: null });
...
export const __grantResponsePacketExportReviewServiceContract = Object.freeze({
  ...
  COMPLETE_GRANT_RESPONSE_PACKET_EXPORT_REVIEW_OPERATION,
  COMPLETE_GRANT_RESPONSE_PACKET_EXPORT_REVIEW_ROLES,
});
