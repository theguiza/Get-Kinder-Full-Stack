  ...
  // P13-EXT-4: funder_outcome_table is generation-time restricted to
  // "funder" only - internal and public are never allowed requestedAudience
  // values for this content type, rejected here at the service-level input
  // validator (the same fail-closed layer isCreateImpactNarrativeDraftInput/
  // isCreateReadinessAssessmentDraftInput/isCreateDataGapMemoDraftInput/
  // isCreateBoardUpdateDraftInput already use to narrow
  // evidence_summary's broader {internal, funder, public} shape down to a
  ...
  export async function createFunderOutcomeTableDraft(input, dependencies = {}) {
    const env = dependencies.env || process.env;
    if (!isKaiSprint2Enabled(env)) return buildKaiError("feature_disabled");
    if (!isKaiGenerationEnabled(env) || !areKaiSprint2GenerationFeaturesEnabled(env)) {
      return buildKaiError("feature_disabled");
    }
    if (!isCreateFunderOutcomeTableDraftInput(input)) {
      return buildKaiError("validation_blocker");
    }
    if (!isMappedHumanActor(input.actorContext)) {
      return buildKaiError("authorization_denied");
    ...
    const repository =
      dependencies.generatedContentRepository || (await createDefaultGeneratedContentRepository());
    const result = await repository.createFunderOutcomeTableDraft(input, {
      draftGenerator: dependencies.draftGenerator,
      metadataOnlyAudit: dependencies.metadataOnlyAudit,
    });
    if (!result.ok) {
