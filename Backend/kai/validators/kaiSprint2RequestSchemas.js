...
const COMPLETE_BOARD_REPORTING_CANDIDATE_REVIEW_REQUEST_KEYS = new Set([
  "expected_updated_at",
]);
...
const COMPLETE_GRANT_RESPONSE_PACKET_EXPORT_REVIEW_REQUEST_KEYS = new Set([
  "expected_updated_at",
]);
...
// P14-07: a Grant Response Packet's audience is always exactly "funder", so
// unlike HUMAN_FINAL_RELEASE_AUTHORITY_REQUEST_KEYS above this accepts no
// requested_audience field at all - matching the exact P14-07B1 repository
// contract, which never accepts one from a caller either.
...
// The packet COMPLETE route requires the same expected_updated_at CAS
// contract as the packet START route above (and as
// validateCompleteExportReviewRequest already enforces for the single-draft
// workflow). No membership, fingerprint, memberCount, manifest identity, or
// approval/final-release decision is accepted.
export function validateCompleteGrantResponsePacketExportReviewRequest(payload) {
  if (!isPlainObject(payload)) {
    return { ok: false, blockers: [requestBlocker("request_body_must_be_object", "body")] };
  }
  ...
  const keys = Object.keys(payload);
  for (const key of keys) {
    if (!COMPLETE_GRANT_RESPONSE_PACKET_EXPORT_REVIEW_REQUEST_KEYS.has(key)) {
      return { ok: false, blockers: [requestBlocker("unknown_field", `body.${key}`)] };
    }
    const value = payload[key];
    if (value === null) return { ok: false, blockers: [requestBlocker("null_field_not_allowed", `body.${key}`)] };
  ...
  for (const key of COMPLETE_GRANT_RESPONSE_PACKET_EXPORT_REVIEW_REQUEST_KEYS) {
    if (!Object.hasOwn(payload, key)) {
      return { ok: false, blockers: [requestBlocker("required_field_missing", `body.${key}`)] };
    }
  }
...
// BR-03B COMPLETE: identical shape to validateStartBoardReportingCandidateReviewRequest
// above - organizationId/engagementId/boardReportingCandidateId/
// reviewQueueItemId all come from the route path; the browser sends back
// only the exact expected_updated_at it last observed on the queue item.
// No membership, fingerprint, member snapshot, release authority,
// eligibility, manifest, or delivery field is accepted.
export function validateCompleteBoardReportingCandidateReviewRequest(payload) {
  if (!isPlainObject(payload)) {
    return { ok: false, blockers: [requestBlocker("request_body_must_be_object", "body")] };
  }
  ...
  const keys = Object.keys(payload);
  for (const key of keys) {
    if (!COMPLETE_BOARD_REPORTING_CANDIDATE_REVIEW_REQUEST_KEYS.has(key)) {
      return { ok: false, blockers: [requestBlocker("unknown_field", `body.${key}`)] };
    }
    const value = payload[key];
    if (value === null) return { ok: false, blockers: [requestBlocker("null_field_not_allowed", `body.${key}`)] };
  ...
  for (const key of COMPLETE_BOARD_REPORTING_CANDIDATE_REVIEW_REQUEST_KEYS) {
    if (!Object.hasOwn(payload, key)) {
      return { ok: false, blockers: [requestBlocker("required_field_missing", `body.${key}`)] };
    }
  }
...
// P14-07: the packet-level analogue of validateHumanFinalReleaseAuthorityRequest
// above - organizationId, engagementId, and the exact candidate id all come
// from the route path, and actorContext/now are server-derived, so this
// route accepts only decision_action, never requested_audience (a Grant
// Response Packet's audience is always exactly "funder"). Rejects any
// fingerprint, members, memberCount, review state, eligibility, authority
// state, or manifest identity.
export function validateGrantResponsePacketHumanFinalReleaseAuthorityRequest(payload) {
  if (!isPlainObject(payload)) {
    return { ok: false, blockers: [requestBlocker("request_body_must_be_object", "body")] };
  }
  ...
  const keys = Object.keys(payload);
  for (const key of keys) {
    if (!GRANT_RESPONSE_PACKET_HUMAN_FINAL_RELEASE_AUTHORITY_REQUEST_KEYS.has(key)) {
      return { ok: false, blockers: [requestBlocker("unknown_field", `body.${key}`)] };
    }
    const value = payload[key];
    if (value === null) return { ok: false, blockers: [requestBlocker("null_field_not_allowed", `body.${key}`)] };
  ...
  for (const key of GRANT_RESPONSE_PACKET_HUMAN_FINAL_RELEASE_AUTHORITY_REQUEST_KEYS) {
    if (!Object.hasOwn(payload, key)) {
      return { ok: false, blockers: [requestBlocker("required_field_missing", `body.${key}`)] };
    }
  }
