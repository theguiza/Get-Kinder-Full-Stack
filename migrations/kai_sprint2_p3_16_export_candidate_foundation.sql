...
-- add only the P3-16 operations, following the same monotonic
-- predecessor-preservation approach established by the repaired P3-04,
-- P3-09, and P3-13 migrations: reconstructing a static historical allowlist
-- here would silently drop any operation the actual validated predecessor
...
BEGIN
  ...
  IF existing_predicate IS NULL THEN
    RAISE EXCEPTION 'validated kai.upload_lifecycle_audit_gate_a_operation_check is required before P3-16 export-candidate-foundation migration';
  END IF;

  IF position('limitation_snapshot_confirmed' IN existing_predicate) = 0
     OR position('export_candidate_created' IN existing_predicate) = 0 THEN
    EXECUTE format(
      'ALTER TABLE kai.upload_lifecycle_audit ADD CONSTRAINT upload_lifecycle_audit_gate_a_operation_check_p3_16 CHECK ((%s) OR operation IN (''limitation_snapshot_confirmed'', ''export_candidate_created'')) NOT VALID',
      existing_predicate
    );
    ALTER TABLE kai.upload_lifecycle_audit
      VALIDATE CONSTRAINT upload_lifecycle_audit_gate_a_operation_check_p3_16;
    ALTER TABLE kai.upload_lifecycle_audit
      DROP CONSTRAINT upload_lifecycle_audit_gate_a_operation_check;
    ALTER TABLE kai.upload_lifecycle_audit
      RENAME CONSTRAINT upload_lifecycle_audit_gate_a_operation_check_p3_16
