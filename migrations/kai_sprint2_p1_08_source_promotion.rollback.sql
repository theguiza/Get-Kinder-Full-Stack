...
-- Drops the P1-08 tables (in child-first order), their indexes, the P1-08-only
-- foreign keys added onto kai.intake_promotion_decisions, and the P1-08-only
-- unique constraints added onto kai.intake_source_candidates and
-- kai.review_queue_items, in each case restoring exactly the pre-P1-08 shape. No
-- earlier-package table, column, index, or constraint is altered beyond that
-- restoration.
ALTER TABLE IF EXISTS kai.intake_promotion_decisions
  DROP CONSTRAINT IF EXISTS intake_promotion_decisions_p1_08_source_version_fk,
  DROP CONSTRAINT IF EXISTS intake_promotion_decisions_p1_08_source_fk;

DROP INDEX IF EXISTS kai.ux_source_versions_p1_08_current_per_source;
DROP TABLE IF EXISTS kai.source_versions;
DROP TABLE IF EXISTS kai.sources;
DROP INDEX IF EXISTS kai.ix_intake_promotion_decisions_p1_08_tenant_candidate;
...
ALTER TABLE kai.intake_source_candidates
  DROP CONSTRAINT IF EXISTS intake_source_candidates_p1_08_promotion_lineage_unique;

ALTER TABLE kai.intake_source_candidates
  DROP CONSTRAINT IF EXISTS intake_source_candidates_p1_08_identity_unique;
