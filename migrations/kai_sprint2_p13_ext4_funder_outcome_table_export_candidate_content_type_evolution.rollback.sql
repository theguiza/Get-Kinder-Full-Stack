...
BEGIN
  IF to_regclass('kai.export_candidates') IS NULL THEN
    RAISE EXCEPTION
      'kai.export_candidates is required before P13-EXT-4 export-candidate rollback';
  END IF;
  ...
  IF actual_def IS NULL THEN
    RAISE EXCEPTION
      'P13-EXT-4 rollback refused: export_candidates_p3_16_content_type_check missing';
  END IF;
  ...
  IF actual_def <> target_def THEN
    RAISE EXCEPTION
      'P13-EXT-4 rollback refused: unexpected constraint definition (%)',
      actual_def;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'P13-EXT-4 rollback refused: funder_outcome_table export candidates exist';
  END IF;
END $$;
