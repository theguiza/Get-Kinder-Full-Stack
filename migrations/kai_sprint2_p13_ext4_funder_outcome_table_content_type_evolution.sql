...
-- P13-EXT-4 adds the eighth generated-content type, funder_outcome_table,
...
-- the exact widening pattern P13-EXT-3 already used to add
-- annual_report_section (itself following P13-EXT-2's pattern for
-- board_update, P13-EXT-1's pattern for case_for_support, and P14-14's
-- pattern for readiness_assessment and data_gap_memo). Without this
-- migration, every funder_outcome_table generation would fail at the
...
-- modified. funder_outcome_table is not added to any Grant Response Packet
-- or Board Reporting membership set - those remain scoped exactly as
-- before, and funder_outcome_table is a distinct, standalone,
-- packet-non-member draft type.

DO $$
BEGIN
  IF to_regclass('kai.generation_runs') IS NULL THEN
    RAISE EXCEPTION 'kai.generation_runs is required before P13-EXT-4 funder_outcome_table content-type evolution migration';
  END IF;
  IF to_regclass('kai.generated_content_drafts') IS NULL THEN
    RAISE EXCEPTION 'kai.generated_content_drafts is required before P13-EXT-4 funder_outcome_table content-type evolution migration';
  END IF;
END $$;

-- Fail closed if the P13-EXT-3 predecessor constraints are not in the exact
...
DECLARE
  runs_def text;
  drafts_def text;
  predecessor_def CONSTANT text := 'CHECK ((content_type = ANY (ARRAY[''evidence_summary''::text, ''impact_narrative''::text, ''readiness_assessment''::text, ''data_gap_memo''::text, ''case_for_support''::text, ''board_update''::text, ''annual_report_section''::text])))';
  target_def CONSTANT text := 'CHECK ((content_type = ANY (ARRAY[''evidence_summary''::text, ''impact_narrative''::text, ''readiness_assessment''::text, ''data_gap_memo''::text, ''case_for_support''::text, ''board_update''::text, ''annual_report_section''::text, ''funder_outcome_table''::text])))';
BEGIN
  ...
  IF runs_def IS NULL THEN
    RAISE EXCEPTION 'generation_runs_p3_01_content_type_check is required (P3-01/P13-01/P14-14/P13-EXT-1/P13-EXT-2/P13-EXT-3) before P13-EXT-4 funder_outcome_table content-type evolution migration';
  END IF;
  IF drafts_def IS NULL THEN
    RAISE EXCEPTION 'generated_content_drafts_p3_01_content_type_check is required (P3-01/P13-01/P14-14/P13-EXT-1/P13-EXT-2/P13-EXT-3) before P13-EXT-4 funder_outcome_table content-type evolution migration';
  END IF;
