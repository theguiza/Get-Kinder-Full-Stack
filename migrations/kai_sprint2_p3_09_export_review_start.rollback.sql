...
DELETE FROM kai.upload_lifecycle_audit
 WHERE operation = 'export_review_started';
...
-- it added "... OR operation = 'export_review_started'" as the outermost
...
-- Mirrors the same rollback mechanism kai_sprint2_p3_05_export_review_request.rollback.sql
-- established for this exact constraint.
DO $p3_09_rollback$
DECLARE
...
BEGIN
  ...
  IF existing_predicate IS NULL THEN
    RAISE EXCEPTION 'validated kai.upload_lifecycle_audit_gate_a_operation_check is required before the P3-09 export-review-start rollback';
  END IF;
  ...
  ELSE
    restored_predicate := regexp_replace(
      existing_predicate,
      '\s*OR\s*\(operation\s*=\s*''export_review_started''(::text)?\)(?=\)*\s*$)',
      ''
    );

    IF restored_predicate = existing_predicate
       OR restored_predicate = ''
       OR position('export_review_started' IN restored_predicate) <> 0
    THEN
      RAISE EXCEPTION 'kai.upload_lifecycle_audit_gate_a_operation_check does not have the expected P3-09 monotonic-extension shape; refusing the P3-09 export-review-start rollback';
    END IF;

    EXECUTE format(
      'ALTER TABLE kai.upload_lifecycle_audit ADD CONSTRAINT upload_lifecycle_audit_gate_a_operation_check_p3_09_rollback CHECK (%s) NOT VALID',
      restored_predicate
    );
    ALTER TABLE kai.upload_lifecycle_audit
      VALIDATE CONSTRAINT upload_lifecycle_audit_gate_a_operation_check_p3_09_rollback;
    ALTER TABLE kai.upload_lifecycle_audit
      DROP CONSTRAINT upload_lifecycle_audit_gate_a_operation_check;
    ALTER TABLE kai.upload_lifecycle_audit
      RENAME CONSTRAINT upload_lifecycle_audit_gate_a_operation_check_p3_09_rollback
