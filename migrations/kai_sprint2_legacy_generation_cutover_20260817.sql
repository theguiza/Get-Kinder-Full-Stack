...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1
     WHERE audit_operation_labels = ARRAY['reserve_upload','start_upload','complete_object_version',
                                          'confirm_upload','block_upload','abandon_upload',
                                          'expire_upload','policy_decision_compare_and_set']::text[]
        OR audit_operation_labels = ARRAY['reserve_upload','start_upload','complete_object_version',
                                           'confirm_upload','block_upload','abandon_upload',
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'kai.review_queue_items.priority is not the supported production enum/default/label shape';
  END IF;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'assertion failed: shared kai.review_queue_items.priority enum/default/labels changed during cutover';
  END IF;
  IF EXISTS (
    SELECT 1 FROM pg_trigger tg
      JOIN pg_class r ON r.oid = tg.tgrelid
