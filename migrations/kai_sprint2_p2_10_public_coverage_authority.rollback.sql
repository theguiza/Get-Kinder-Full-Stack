...
BEGIN
  ...
  IF old_expr IS NULL THEN
    RAISE EXCEPTION 'kai.upload_lifecycle_audit_gate_a_operation_check is required before P2-10 public coverage-authority rollback';
  END IF;
  ...
  SELECT array_agg(DISTINCT op ORDER BY op)
   ...
   WHERE op <> 'coverage_review_decision_accepted_public_with_limitation';
