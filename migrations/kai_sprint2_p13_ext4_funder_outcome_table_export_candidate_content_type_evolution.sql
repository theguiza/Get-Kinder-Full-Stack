...
-- P13-EXT-4 companion to kai_sprint2_p13_ext4_funder_outcome_table_content_type_evolution.sql:
-- widens kai.export_candidates' own content_type CHECK constraint to include
-- funder_outcome_table, following the exact pattern P13-EXT-3 already used
-- to add annual_report_section (EXPORT_CANDIDATE_CONTENT_TYPES in
...
BEGIN
  IF to_regclass('kai.export_candidates') IS NULL THEN
    RAISE EXCEPTION
      'kai.export_candidates is required before P13-EXT-4 funder_outcome_table export-candidate content-type evolution migration';
  END IF;
  ...
  IF candidate_def IS NULL THEN
    RAISE EXCEPTION
      'export_candidates_p3_16_content_type_check is required (P3-16/P14-15/P13-EXT-1/P13-EXT-2/P13-EXT-3) before P13-EXT-4';
  END IF;
