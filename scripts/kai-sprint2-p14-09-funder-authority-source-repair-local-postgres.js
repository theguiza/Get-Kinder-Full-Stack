...
const dbName = "kai_p14_09_funder_authority_source_repair_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p14-09-far-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P14-09 funder-authority-repair runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P14-09 funder-authority-repair runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P14-09 funder-authority-repair runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P14-09 funder-authority-repair runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  psqlExec("ALTER TABLE kai.engagements ADD CONSTRAINT kai_p14_09_funder_authority_engagements_id_org_unique UNIQUE (engagement_id, organization_id);");
  ...
  // P3-01 (immediately above) overwrites
  ...
  // exists to fix, so it must run directly after P2-11.
  ...
  const testResult = spawnSync("node", [
    ...
    env: {
      ...
      DB_USER: user,
      DB_PASSWORD: "",
      KAI_P14_09_FUNDER_AUTHORITY_DATABASE_URL: targetUrl,
    },
  });
  if (testResult.status !== 0) throw new Error("P14-09 funder-authority-source-repair tests failed");
  console.log("P14-09 funder-authority-source-repair focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P14-09 funder-authority-source-repair ephemeral PostgreSQL workdir removed: ${workDir}`);
}
