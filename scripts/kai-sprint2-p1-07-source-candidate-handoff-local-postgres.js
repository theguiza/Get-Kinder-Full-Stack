...
const dbName = "kai_p1_07_source_candidate_handoff_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p1-07-handoff-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P1-07 handoff runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P1-07 handoff runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P1-07 handoff runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P1-07 handoff runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  console.log(`P1-07 source-candidate handoff ephemeral database created: ${dbName}`);
  console.log(`P1-07 source-candidate handoff ephemeral PostgreSQL loopback: 127.0.0.1:${port}`);
  ...
  const testResult = spawnSync("node", [
    "--test",
    "__tests__/kai-sprint2-p1-07-source-candidate-handoff.integration.spec.js",
  ], {
    cwd: repoRoot,
    encoding: "utf8",
    stdio: "inherit",
    env: {
      ...
      DB_USER: user,
      DB_PASSWORD: "",
      KAI_P1_07_SOURCE_CANDIDATE_HANDOFF_DATABASE_URL: targetUrl,
    },
  });
  if (testResult.status !== 0) throw new Error("P1-07 source-candidate handoff integration tests failed");
  console.log("P1-07 source-candidate handoff integration tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P1-07 source-candidate handoff ephemeral PostgreSQL workdir removed: ${workDir}`);
}
