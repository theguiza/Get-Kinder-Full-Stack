...
const dbName = "kai_p1_07_source_candidate_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p1-07-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P1-07 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P1-07 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P1-07 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P1-07 runner refused non-loopback listen_addresses");
    if (row.version_num < 160000 || row.version_num >= 170000) throw new Error("P1-07 runner requires PostgreSQL 16");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  console.log(`P1-07 source-candidate ephemeral database created: ${dbName}`);
  console.log(`P1-07 source-candidate ephemeral PostgreSQL loopback: 127.0.0.1:${port}`);
  ...
  const catalogOutput = psqlFile("scripts/kai-sprint2-p1-07-source-candidate-verifier.sql");
  console.log(catalogOutput);
  assertNoFail("P1-07 catalog verifier", catalogOutput);

  const failureOutput = psqlFile("scripts/kai-sprint2-p1-07-source-candidate-failure-checks.sql");
  console.log(failureOutput);
  assertNoFail("P1-07 read-only failure checks", failureOutput);
  ...
  const smokeOutput = psqlFile("scripts/kai-sprint2-p1-07-source-candidate-smoke-verifier.sql");
  console.log(smokeOutput);
  assertNoFail("P1-07 smoke verifier", smokeOutput);
  ...
  const testEnv = {
    ...
    DB_USER: user,
    DB_PASSWORD: "",
    KAI_P1_07_SOURCE_CANDIDATE_DATABASE_URL: targetUrl,
  };
  for (const spec of [
    ...
    if (testResult.status !== 0) throw new Error(`P1-07 source-candidate integration tests failed: ${spec}`);
  }
  console.log("P1-07 source-candidate integration tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P1-07 source-candidate ephemeral PostgreSQL workdir removed: ${workDir}`);
}
