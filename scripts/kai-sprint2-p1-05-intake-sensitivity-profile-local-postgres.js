...
const workDir = mkdtempSync(join(tmpdir(), "kai-p1-05-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P1-05 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P1-05 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P1-05 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P1-05 runner refused non-loopback listen_addresses");
    if (row.version_num < 160000 || row.version_num >= 170000) throw new Error("P1-05 runner requires PostgreSQL 16");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  console.log(`P1-05 intake-sensitivity-profile ephemeral database created: ${dbName}`);
  console.log(`P1-05 intake-sensitivity-profile ephemeral PostgreSQL loopback: 127.0.0.1:${port}`);
  ...
  const testEnv = {
    ...
    DB_USER: user,
    DB_PASSWORD: "",
    KAI_P1_05_SENSITIVITY_PROFILE_DATABASE_URL: targetUrl,
  };
  for (const spec of [
    "__tests__/kai-sprint2-p1-05-intake-sensitivity-profile.integration.spec.js",
  ]) {
    ...
    if (testResult.status !== 0) throw new Error(`P1-05 intake-sensitivity-profile integration tests failed: ${spec}`);
  }
  console.log("P1-05 intake-sensitivity-profile integration tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P1-05 intake-sensitivity-profile ephemeral PostgreSQL workdir removed: ${workDir}`);
}
