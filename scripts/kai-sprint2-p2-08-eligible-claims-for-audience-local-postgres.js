...
const dbName = "kai_p2_08_eligible_claims_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p2-08-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  if (!["127.0.0.1", "localhost", "::1"].includes(parsed.hostname.toLowerCase())) {
    throw new Error("P2-08 runner refused non-loopback target before connection");
  }
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P2-08 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P2-08 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P2-08 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P2-08 runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  const testResult = spawnSync("node", [
    ...
    env: {
      ...
      DB_USER: user,
      DB_PASSWORD: "",
      KAI_P2_08_ELIGIBLE_CLAIMS_DATABASE_URL: targetUrl,
    },
  });
  if (testResult.status !== 0) throw new Error("P2-08 eligible-claims tests failed");
  console.log("P2-08 eligible-claims focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P2-08 eligible-claims ephemeral PostgreSQL workdir removed: ${workDir}`);
}
