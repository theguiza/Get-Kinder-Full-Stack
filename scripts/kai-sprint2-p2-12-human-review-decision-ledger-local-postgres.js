...
const dbName = "kai_p2_12_human_review_decision_ledger_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p2-12-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P2-12 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P2-12 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P2-12 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P2-12 runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  psqlFile("scripts/kai-sprint2-p2-12-human-review-decision-ledger-verifier.sql");
  ...
  const testResult = spawnSync("node", [
    "--test",
    "__tests__/kai-sprint2-p2-12-human-review-decision-ledger-integration.spec.js",
    ...
    env: {
      ...
      DB_USER: user,
      DB_PASSWORD: "",
      KAI_P2_12_HUMAN_REVIEW_DECISION_LEDGER_DATABASE_URL: targetUrl,
    },
  });
  if (testResult.status !== 0) throw new Error("P2-12 human-review-decision-ledger tests failed");
  console.log("P2-12 human-review-decision-ledger focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P2-12 human-review-decision-ledger ephemeral PostgreSQL workdir removed: ${workDir}`);
}
