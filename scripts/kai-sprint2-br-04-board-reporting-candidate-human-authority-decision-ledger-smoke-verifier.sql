DROP TABLE IF EXISTS br_04_smoke_results;
CREATE TEMP TABLE br_04_smoke_results (
  check_name text PRIMARY KEY,
  status text NOT NULL,
  detail text NOT NULL
);

INSERT INTO br_04_smoke_results
SELECT 'candidate_a_root_grant_is_current_head',
       CASE WHEN EXISTS (
              SELECT 1
                FROM kai.board_reporting_candidate_human_authority_decisions d
               WHERE d.board_reporting_candidate_id = '15030000-0000-4000-8000-000000000323'
                 AND d.decision_id = '15070000-0000-4000-8000-000000000601'
...
SELECT 'candidate_b_has_no_decision',
       CASE WHEN NOT EXISTS (
              SELECT 1 FROM kai.board_reporting_candidate_human_authority_decisions
...
INSERT INTO kai.board_reporting_candidate_human_authority_decisions (
  organization_id, board_reporting_candidate_id, decision_type, decision_action,
       ...
       CASE WHEN EXISTS (
              SELECT 1
               ...
               WHERE d.board_reporting_candidate_id = '15030000-0000-4000-8000-000000000323'
                 ...
                 AND d.supersedes_decision_id = '15070000-0000-4000-8000-000000000601'
                 AND NOT EXISTS (SELECT 1 FROM kai.board_reporting_candidate_human_authority_decisions s WHERE s.supersedes_decision_id = d.decision_id)
            ) THEN 'PASS' ELSE 'FAIL' END,
...
SELECT 'old_grant_head_is_no_longer_current',
       CASE WHEN NOT EXISTS (
              SELECT 1
                FROM kai.board_reporting_candidate_human_authority_decisions d
               WHERE d.decision_id = '15070000-0000-4000-8000-000000000601'
                 AND NOT EXISTS (SELECT 1 FROM kai.board_reporting_candidate_human_authority_decisions s WHERE s.supersedes_decision_id = d.decision_id)
            )
            THEN 'PASS' ELSE 'FAIL' END,
       'the superseded grant row is never mutated, but it is no longer the lineage head';

SELECT * FROM br_04_smoke_results ORDER BY check_name;

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM br_04_smoke_results WHERE status <> 'PASS') THEN
    RAISE EXCEPTION 'BR-04 board-reporting-candidate-human-authority-decision-ledger smoke verifier failed';
  END IF;
END $$;
