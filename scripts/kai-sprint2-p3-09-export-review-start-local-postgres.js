...
const dbName = "kai_p3_09_export_review_start_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p3-09-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  if (!["127.0.0.1", "localhost", "::1"].includes(parsed.hostname.toLowerCase())) {
    throw new Error("P3-09 runner refused non-loopback target before connection");
  }
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P3-09 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P3-09 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P3-09 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P3-09 runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  psqlExec(
    "ALTER TABLE kai.engagements ADD CONSTRAINT kai_p3_09_engagements_id_org_unique UNIQUE (engagement_id, organization_id);",
  );
  ...
  // The P3-05 verifier asserts the single-state export_review contract check by
  // name; P3-09 intentionally replaces that constraint (see its verifier's
  ...
  // Real kai.organizations/kai.engagements rows the P3-09 integration suite's
  ...
  // hardcodes.
  psqlExec(
    "INSERT INTO kai.organizations (organization_id, name, organization_code) VALUES ('00000000-0000-4000-8000-000000000001', 'P3-09 Smoke Org', 'p3-09-smoke-org') ON CONFLICT (organization_id) DO NOTHING;",
  );
  psqlExec(
  ...
  const testResult = spawnSync("node", [
    ...
    env: {
      ...
      DB_USER: user,
      DB_PASSWORD: "",
      KAI_P3_09_EXPORT_REVIEW_START_DATABASE_URL: targetUrl,
    },
  });
  if (testResult.status !== 0) throw new Error("P3-09 export-review-start tests failed");
  console.log("P3-09 export-review-start focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P3-09 export-review-start ephemeral PostgreSQL workdir removed: ${workDir}`);
}
