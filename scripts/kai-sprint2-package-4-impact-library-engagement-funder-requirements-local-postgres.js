...
const dbName = "kai_package_4_funder_requirements_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p4-fr-pg-"));
const dataDir = join(workDir, "data");
const socketDir = join(workDir, "socket");
const logFile = join(workDir, "postgres.log");
const port = String(59300 + Math.floor(Math.random() * 1000));
...
async function proveRunnerOwnedTarget() {
  ...
  if (!["127.0.0.1", "localhost", "::1"].includes(parsed.hostname.toLowerCase())) {
    throw new Error("Package 4 assembled-proof runner refused a non-loopback target before connecting");
  }
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("Package 4 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`Package 4 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("Package 4 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("Package 4 runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  // Full dependency chain required for the REAL Package 3A/3B ir_contrib_002
  // assessment rule to run unmocked (its loadInputs reads evidence/claim/
  // gap/impact-evaluation tables even when empty) - the exact, already-
  // proven chain scripts/kai-sprint2-c3-b3-seven-rules-local-postgres.js
  // assembles, reused verbatim. Package 2A's own authority migration and
  // this proof's additive auth-table mirror (so the REAL
  // resolveKaiActorContext can run unmocked) are the only additions.
  psqlFile("scripts/kai-sprint2-organization-enablement-bootstrap-synthetic-schema.sql");
  psqlExec(
    "ALTER TABLE kai.engagements ADD CONSTRAINT kai_p4_fr_engagements_id_org_unique UNIQUE (engagement_id, organization_id);",
  );
  ...
  // The assembled proof invokes the REAL route handler and REAL services
  // with ZERO dependency overrides, so every one of their internal
  // kaiQueries.js/kaiDb.js calls resolves against the ambient, module-level
  // production pool (Backend/db/pg.js) - never a test-owned pool passed as
  // an explicit dependency. That ambient pool is a singleton fixed at
  // module-import time from environment, so this child process is launched
  // with no DATABASE_URL/DATABASE_URL_LOCAL/PGURL_LOCAL/RENDER_DATABASE_URL/
  // PROD_DATABASE_URL set at all (every one of those is explicitly cleared)
  // and only discrete DB_HOST/DB_PORT/DB_NAME/DB_USER/DB_PASSWORD naming the
  // loopback-only ephemeral instance above - so the real, unmocked
  // production code path connects only to this disposable synthetic
  // database. A second, explicit, runner-owned connection string
  // (KAI_PACKAGE_4_FUNDER_REQUIREMENTS_DATABASE_URL) is passed separately
  // for the proof's own fixture-seeding/assertion queries.
  const testResult = spawnSync("node", [
    "--test",
    "__tests__/kai-sprint2-package-4-impact-library-engagement-funder-requirements.integration.spec.js",
  ], {
    cwd: repoRoot,
    encoding: "utf8",
    stdio: "inherit",
    env: {
      ...
      DB_USER: user,
      DB_PASSWORD: "",
      KAI_PACKAGE_4_FUNDER_REQUIREMENTS_DATABASE_URL: targetUrl,
      KAI_SPRINT2_ENABLED: "true",
    },
  });
  if (testResult.status !== 0) throw new Error("Package 4 assembled Funder Requirements proof failed");
  console.log("Package 4 assembled /impact-library Funder Requirements local PostgreSQL proof passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`Package 4 assembled-proof ephemeral PostgreSQL workdir removed: ${workDir}`);
}
