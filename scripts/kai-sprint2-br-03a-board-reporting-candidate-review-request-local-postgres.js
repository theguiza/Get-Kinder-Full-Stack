...
const dbName = "kai_br_03a_board_reporting_candidate_review_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-br-03a-pg-"));
...
const EXPECTED_CHECKS = [
  ...
  "board_review_contract_check_present",
...
function proveVerifierOutput(path) {
  ...
  if (lines[0] !== "check_name,status,detail") {
    throw new Error(`BR-03A verifier output contract violated: ${lines[0]}`);
  }
  ...
  for (const row of dataRows) {
    ...
    seen.add(checkName);
    if (status !== "PASS") throw new Error(`BR-03A verifier check ${checkName} is ${status}`);
  }
  for (const checkName of EXPECTED_CHECKS) {
    if (!seen.has(checkName)) throw new Error(`BR-03A verifier missing ${checkName}`);
  }
  console.log(`BR-03A verifier output contract proven: exactly ${dataRows.length} PASS rows.`);
}
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("BR-03A runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`BR-03A runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("BR-03A runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("BR-03A runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
function installRuntimeMigrationChain() {
  ...
  psqlExec("ALTER TABLE kai.engagements ADD CONSTRAINT kai_br_03a_engagements_id_org_unique UNIQUE (engagement_id, organization_id);");
...
try {
  ...
  proveVerifierOutput("scripts/kai-sprint2-br-03a-board-reporting-candidate-review-request-verifier.sql");

  psqlFile("scripts/kai-sprint2-br-03a-board-reporting-candidate-review-request-smoke-seed.sql");
  psqlFile("scripts/kai-sprint2-br-03a-board-reporting-candidate-review-request-smoke-verifier.sql");
  psqlFile("scripts/kai-sprint2-br-03a-board-reporting-candidate-review-request-failure-checks.sql");
  psqlExec("DELETE FROM kai.review_queue_items WHERE queue_type = 'board_reporting_candidate_review';");
  psqlFile("migrations/kai_sprint2_br_03a_board_reporting_candidate_review_request.rollback.sql");
  const afterRollback = psqlExec("SELECT pg_get_constraintdef(oid) AS def FROM pg_constraint WHERE conrelid = 'kai.review_queue_items'::regclass AND conname = 'review_queue_items_p1_06_queue_type_check';");
  if (afterRollback.includes("board_reporting_candidate_review")) {
    throw new Error("BR-03A rollback left board_reporting_candidate_review admitted");
  }
  psqlFile("migrations/kai_sprint2_br_03a_board_reporting_candidate_review_request.sql");
  proveVerifierOutput("scripts/kai-sprint2-br-03a-board-reporting-candidate-review-request-verifier.sql");
  console.log("BR-03A forward -> rollback -> forward proof against synthetic local PostgreSQL passed.");
  ...
  const testResult = spawnSync("node", [
    ...
    env: {
      ...
      DB_USER: user,
      DB_PASSWORD: "",
      KAI_BR_03A_BOARD_REPORTING_CANDIDATE_REVIEW_REQUEST_DATABASE_URL: targetUrl,
    },
  });
  if (testResult.status !== 0) throw new Error("BR-03A board-reporting-candidate-review tests failed");
  console.log("BR-03A board-reporting-candidate-review focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`BR-03A board-reporting-candidate-review ephemeral PostgreSQL workdir removed: ${workDir}`);
}
