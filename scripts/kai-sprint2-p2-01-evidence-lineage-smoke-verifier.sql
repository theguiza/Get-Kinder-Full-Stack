...
BEGIN
  ...
  -- Creation: one field-presence fact per committed kai.data_dictionary_fields row
  -- with its own 'column' locator, non-null source_id, sensitivity_level copied
  -- from the field's own committed sensitivity, support_strength 'unassessed', and
  -- one open evidence_review queue item per evidence item, all atomically.
  ...
  VALUES
    (field1_evidence_id, org1, source1, source_version1, field1_locator_id, 'dictionary_field_presence_fact', 'organization_committed_metadata', 'unknown', 'unassessed', field1_statement, field1_fingerprint, 'human'),
    (field2_evidence_id, org1, source1, source_version1, field2_locator_id, 'dictionary_field_presence_fact', 'organization_committed_metadata', 'unknown', 'unassessed', field2_statement, field2_fingerprint, 'human');
  ...
  INSERT INTO p2_01_results VALUES (
    ...
    CASE WHEN (SELECT count(*) FROM kai.evidence_items WHERE organization_id = org1 AND source_version_id = source_version1) = 2
      THEN 'PASS' ELSE 'FAIL' END,
    'exactly two evidence items (one per committed field, each locator-bound) for the accepted source_version identity'
  );
  INSERT INTO p2_01_results VALUES (
  ...
  INSERT INTO p2_01_results VALUES (
    ...
    CASE WHEN (SELECT count(*) FROM kai.review_queue_items WHERE organization_id = org1 AND queue_type = 'evidence_review' AND queue_status = 'open'
                 AND target_object_id IN (field1_evidence_id, field2_evidence_id)) = 2
      THEN 'PASS' ELSE 'FAIL' END,
    'exactly two open evidence_review queue items, one per evidence item'
  );
  INSERT INTO p2_01_results VALUES (
    'creation_evidence_review_required_action_set',
    CASE WHEN NOT EXISTS (
      SELECT 1 FROM kai.review_queue_items
       WHERE organization_id = org1 AND queue_type = 'evidence_review'
         AND target_object_id IN (field1_evidence_id, field2_evidence_id)
         AND (required_action IS NULL OR btrim(required_action) = '' OR required_action <> required_action1)
    ) THEN 'PASS' ELSE 'FAIL' END,
    'every evidence_review queue item carries the exact required disclosed required_action text'
  );
  INSERT INTO p2_01_results VALUES (
    'creation_evidence_lineage_tuple_bound',
    CASE WHEN NOT EXISTS (
      SELECT 1 FROM kai.evidence_items
       WHERE organization_id = org1 AND source_version_id = source_version1
         AND (source_id IS DISTINCT FROM source1 OR source_locator_id IS NULL)
    ) THEN 'PASS' ELSE 'FAIL' END,
    'every evidence item carries the exact source_id/source_version_id/organization_id lineage tuple and a non-null source_locator_id'
  );
  INSERT INTO p2_01_results VALUES (
  ...
  INSERT INTO p2_01_results VALUES (
    ...
    CASE WHEN (SELECT evidence_item_id FROM kai.evidence_items WHERE organization_id = org1 AND source_version_id = source_version1 AND statement_fingerprint = field1_fingerprint) = field1_evidence_id
      THEN 'PASS' ELSE 'FAIL' END,
  ...
  BEGIN
    ...
    VALUES (org1, source1, source_version1, field1_locator_id, 'dictionary_field_presence_fact', 'organization_committed_metadata', 'unknown', 'unassessed', field1_statement, field1_fingerprint);
  ...
  INSERT INTO p2_01_results VALUES (
    ...
    CASE WHEN (SELECT count(*) FROM kai.review_queue_items WHERE organization_id = org1 AND queue_type = 'evidence_review' AND target_object_id = field1_evidence_id) = 1
      THEN 'PASS' ELSE 'FAIL' END,
    ...
    BEGIN
      ...
      VALUES (
        ...
        jsonb_build_object(
          ...
          'profile_canonical_sha256', repeat('d', 64), 'evidence_item_count', 3, 'source_locator_count', 3,
          'review_queue_item_count', 3, 'fresh_write_count', 1, 'validator_key', 'VAL-KAI-P2-01-001'
        )
      );
