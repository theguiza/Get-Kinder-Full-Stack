...
import { assertNoFail } from "./kai-sprint2-p2-04-claim-gap-followup-runner-assertions.js";

const repoRoot = new URL("..", import.meta.url).pathname.replace(/\/$/, "");
const dbName = "kai_p2_04_claim_gap_followup_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p2-04-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P2-04 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P2-04 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P2-04 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P2-04 runner refused non-loopback listen_addresses");
    if (row.version_num < 160000 || row.version_num >= 170000) throw new Error("P2-04 runner requires PostgreSQL 16");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  console.log(`P2-04 claim-gap/client-followup ephemeral database created: ${dbName}`);
  console.log(`P2-04 claim-gap/client-followup ephemeral PostgreSQL loopback: 127.0.0.1:${port}`);
  ...
  const catalogOutput = psqlFile("scripts/kai-sprint2-p2-04-claim-gap-followup-verifier.sql");
  console.log(catalogOutput);
  assertNoFail("P2-04 catalog verifier", catalogOutput);

  const failureOutput = psqlFile("scripts/kai-sprint2-p2-04-claim-gap-followup-failure-checks.sql");
  console.log(failureOutput);
  assertNoFail("P2-04 read-only failure checks", failureOutput);
  ...
  psqlFile("scripts/kai-sprint2-p2-04-claim-gap-followup-smoke-seed.sql");

  const smokeOutput = psqlFile("scripts/kai-sprint2-p2-04-claim-gap-followup-smoke-verifier.sql");
  console.log(smokeOutput);
  assertNoFail("P2-04 smoke verifier", smokeOutput);
  ...
  const testEnv = {
    ...
    DB_USER: user,
    DB_PASSWORD: "",
    KAI_P2_04_CLAIM_GAP_FOLLOWUP_DATABASE_URL: targetUrl,
  };
  for (const spec of [
    ...
    if (testResult.status !== 0) throw new Error(`P2-04 claim-gap/client-followup integration tests failed: ${spec}`);
  }
  console.log("P2-04 claim-gap/client-followup integration tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P2-04 claim-gap/client-followup ephemeral PostgreSQL workdir removed: ${workDir}`);
}
