...
const dbName = "kai_p3_18_real_persisted_final_gate_proof_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p3-18-real-gate-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  if (!["127.0.0.1", "localhost", "::1"].includes(parsed.hostname.toLowerCase())) {
    throw new Error("P3-18 real-persisted-final-gate-proof runner refused non-loopback target before connection");
  }
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P3-18 real-persisted-final-gate-proof runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P3-18 real-persisted-final-gate-proof runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P3-18 real-persisted-final-gate-proof runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P3-18 real-persisted-final-gate-proof runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  psqlExec(
    "ALTER TABLE kai.engagements ADD CONSTRAINT kai_p3_18_engagements_id_org_unique UNIQUE (engagement_id, organization_id);",
  );
  ...
  // re-run; each superseding package's own verifier asserts that
  // replacement.
  ...
  // createImpactNarrativeDraft calls now require as the requested
  // engagementId (P14-01 write contract), reusing the exact UUID constants
  // __tests__/kai-sprint2-p3-18-real-persisted-final-gate-proof.integration.spec.js
  // and __tests__/kai-sprint2-p3-13-export-review-completion.integration.spec.js
  // each hardcode.
  psqlExec(
    "INSERT INTO kai.organizations (organization_id, name, organization_code) VALUES ('00000000-0000-4000-8000-000000000001', 'P3-18 Smoke Org', 'p3-18-smoke-org') ON CONFLICT (organization_id) DO NOTHING;",
  );
  psqlExec(
    "INSERT INTO kai.engagements (engagement_id, organization_id, engagement_code) VALUES ('00000000-0000-4000-8000-000000000918', '00000000-0000-4000-8000-000000000001', 'p3-18-smoke-engagement') ON CONFLICT (engagement_id) DO NOTHING;",
  );
  psqlExec(
  ...
  if (testResult.status !== 0) throw new Error("P3-18 real-persisted-final-gate-proof tests failed");
  console.log("P3-18 real-persisted-final-gate-proof focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P3-18 real-persisted-final-gate-proof ephemeral PostgreSQL workdir removed: ${workDir}`);
}
