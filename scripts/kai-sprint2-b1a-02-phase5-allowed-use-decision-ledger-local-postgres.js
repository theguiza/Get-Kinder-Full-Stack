...
const dbName = "kai_b1a_02_phase5_allowed_use_decision_ledger_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-b1a-02-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("B1A-2 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`B1A-2 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("B1A-2 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("B1A-2 runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  psqlFile("scripts/kai-sprint2-b1a-02-phase5-allowed-use-decision-ledger-verifier.sql");
  ...
  const testResult = spawnSync("node", [
    "--test",
    "__tests__/kai-sprint2-b1a-02-phase5-allowed-use-decision-ledger-integration.spec.js",
    "__tests__/kai-sprint2-b1a-02-phase5-allowed-use-decision-ledger-boundary.spec.js",
  ], {
    cwd: repoRoot,
    encoding: "utf8",
    stdio: "inherit",
    env: {
      ...
      DB_USER: user,
      DB_PASSWORD: "",
      KAI_B1A_02_PHASE5_ALLOWED_USE_DECISION_LEDGER_DATABASE_URL: targetUrl,
    },
  });
  if (testResult.status !== 0) throw new Error("B1A-2 phase-5-allowed-use-decision-ledger tests failed");
  console.log("B1A-2 phase-5-allowed-use-decision-ledger focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`B1A-2 phase-5-allowed-use-decision-ledger ephemeral PostgreSQL workdir removed: ${workDir}`);
}
