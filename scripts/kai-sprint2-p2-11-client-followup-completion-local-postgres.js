...
const dbName = "kai_p2_11_client_followup_completion_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p2-11-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P2-11 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P2-11 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P2-11 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P2-11 runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  const testResult = spawnSync("node", [
    "--test",
    "__tests__/kai-sprint2-p2-11-client-followup-completion.integration.spec.js",
    ...
    env: {
      ...
      DB_USER: user,
      DB_PASSWORD: "",
      KAI_P2_11_CLIENT_FOLLOWUP_COMPLETION_DATABASE_URL: targetUrl,
    },
  });
  if (testResult.status !== 0) throw new Error("P2-11 client-followup-completion tests failed");
  console.log("P2-11 client-followup-completion focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P2-11 client-followup-completion ephemeral PostgreSQL workdir removed: ${workDir}`);
}
