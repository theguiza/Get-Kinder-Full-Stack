...
const dbName = "kai_p3_06_export_review_packet_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p3-06-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  if (!["127.0.0.1", "localhost", "::1"].includes(parsed.hostname.toLowerCase())) {
    throw new Error("P3-06 runner refused non-loopback target before connection");
  }
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P3-06 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P3-06 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P3-06 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P3-06 runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  psqlExec(
    "ALTER TABLE kai.engagements ADD CONSTRAINT kai_p3_06_engagements_id_org_unique UNIQUE (engagement_id, organization_id);",
  );
  ...
  // Real kai.organizations/kai.engagements rows the P3-06 integration suite's
  ...
  // hardcodes.
  psqlExec(
    "INSERT INTO kai.organizations (organization_id, name, organization_code) VALUES ('00000000-0000-4000-8000-000000000001', 'P3-06 Smoke Org', 'p3-06-smoke-org') ON CONFLICT (organization_id) DO NOTHING;",
  );
  psqlExec(
    "INSERT INTO kai.engagements (engagement_id, organization_id, engagement_code) VALUES ('00000000-0000-4000-8000-000000000906', '00000000-0000-4000-8000-000000000001', 'p3-06-smoke-engagement') ON CONFLICT (engagement_id) DO NOTHING;",
  );
  ...
  const testResult = spawnSync("node", [
    ...
    env: {
      ...
      DB_USER: user,
      DB_PASSWORD: "",
      KAI_P3_06_EXPORT_REVIEW_PACKET_DATABASE_URL: targetUrl,
    },
  });
  if (testResult.status !== 0) throw new Error("P3-06 export-review-packet tests failed");
  console.log("P3-06 export-review-packet focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P3-06 export-review-packet ephemeral PostgreSQL workdir removed: ${workDir}`);
}
