...
const dbName = "kai_br_02_board_reporting_candidate_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-br-02-pg-"));
...
const EXPECTED_CHECKS = [
  ...
  "candidate_engagement_fk_is_tenant_safe",
  "candidate_internal_audience_check_present",
  "candidate_idempotency_unique_present",
  "candidate_fingerprint_contract_check_present",
  "candidate_fingerprint_format_check_present",
  "candidate_append_only_trigger_present",
  "member_candidate_fk_is_tenant_safe",
  "member_draft_fk_is_tenant_safe",
...
function proveVerifierOutput(path) {
  ...
  if (lines[0] !== "check_name,status,detail") {
    throw new Error(`BR-02 verifier output contract violated: ${lines[0]}`);
  }
  const dataRows = lines.slice(1);
  if (dataRows.length !== EXPECTED_CHECKS.length) {
    throw new Error(`BR-02 verifier expected ${EXPECTED_CHECKS.length} rows, got ${dataRows.length}`);
  }
  const seen = new Set();
  for (const row of dataRows) {
    const [checkName, status] = row.split(",");
    if (seen.has(checkName)) throw new Error(`BR-02 duplicate verifier check ${checkName}`);
    seen.add(checkName);
    if (status !== "PASS") throw new Error(`BR-02 verifier check ${checkName} is ${status}`);
  }
  for (const checkName of EXPECTED_CHECKS) {
    if (!seen.has(checkName)) throw new Error(`BR-02 verifier missing ${checkName}`);
  }
  console.log(`BR-02 verifier output contract proven: exactly ${dataRows.length} PASS rows.`);
}
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("BR-02 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`BR-02 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("BR-02 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("BR-02 runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  proveVerifierOutput("scripts/kai-sprint2-br-02-board-reporting-candidate-foundation-verifier.sql");
  psqlFile("scripts/kai-sprint2-br-02-board-reporting-candidate-foundation-smoke-seed.sql");
  psqlFile("scripts/kai-sprint2-br-02-board-reporting-candidate-foundation-smoke-verifier.sql");
  psqlFile("scripts/kai-sprint2-br-02-board-reporting-candidate-foundation-failure-checks.sql");

  const testResult = spawnSync("node", [
    "--test",
    "__tests__/kai-sprint2-br-02-board-reporting-candidate-foundation.integration.spec.js",
    ...
    env: {
      ...
      DB_USER: user,
      DB_PASSWORD: "",
      KAI_BR_02_BOARD_REPORTING_CANDIDATE_FOUNDATION_DATABASE_URL: targetUrl,
    },
  });
  if (testResult.status !== 0) throw new Error("BR-02 board-reporting-candidate tests failed");
  console.log("BR-02 board-reporting-candidate focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`BR-02 board-reporting-candidate ephemeral PostgreSQL workdir removed: ${workDir}`);
}
