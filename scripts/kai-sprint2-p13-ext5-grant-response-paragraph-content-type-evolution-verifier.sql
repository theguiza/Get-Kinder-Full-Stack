-- KAI P13-EXT-5 grant_response_paragraph content-type evolution catalog verifier.
--
...
-- constraint definition; a mixed predecessor/target state; grant_response_paragraph
...
results AS (
  ...
  SELECT
    label || '_at_target_grant_response_paragraph_vocabulary',
    CASE WHEN definition_state = 'target' THEN 'PASS' ELSE 'FAIL' END,
    CASE
      WHEN definition_state = 'missing' THEN constraint_name || ' is missing'
      WHEN definition_state = 'target' THEN constraint_name || ' is at the exact P13-EXT-5 target vocabulary'
      WHEN definition_state = 'predecessor' THEN constraint_name || ' is still at the P13-EXT-4 predecessor vocabulary'
  ...
  SELECT
    ...
    'all three constraints must be at the same target grant_response_paragraph-inclusive state'
  FROM classified_state

  UNION ALL

  SELECT
    label || '_grant_response_paragraph_present_when_at_target',
    CASE
      WHEN definition_state = 'target' AND actual_def LIKE '%''grant_response_paragraph''::text%' THEN 'PASS'
      ELSE 'FAIL'
    END,
    constraint_name || ' includes grant_response_paragraph when reporting the target vocabulary'
  FROM classified_state
...
BEGIN
  IF EXISTS (
    WITH constraint_state(label, table_regclass, constraint_name, predecessor_def, target_def) AS (
      VALUES
        ('generation_runs', 'kai.generation_runs', 'generation_runs_p3_01_content_type_check', 'CHECK ((content_type = ANY (ARRAY[''evidence_summary''::text, ''impact_narrative''::text, ''readiness_assessment''::text, ''data_gap_memo''::text, ''case_for_support''::text, ''board_update''::text, ''annual_report_section''::text, ''funder_outcome_table''::text])))', 'CHECK ((content_type = ANY (ARRAY[''evidence_summary''::text, ''impact_narrative''::text, ''readiness_assessment''::text, ''data_gap_memo''::text, ''case_for_support''::text, ''board_update''::text, ''annual_report_section''::text, ''funder_outcome_table''::text, ''grant_response_paragraph''::text])))'),
    ...
    failures AS (
      ...
      SELECT 1
      FROM classified_state
      WHERE actual_def NOT LIKE '%''grant_response_paragraph''::text%'
      UNION ALL
      SELECT 1
    ...
    RAISE EXCEPTION 'P13-EXT-5 grant_response_paragraph content-type evolution verifier failed';
  END IF;
END $$;
