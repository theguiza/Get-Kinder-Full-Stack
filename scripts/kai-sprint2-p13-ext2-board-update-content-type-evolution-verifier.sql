-- KAI P13-EXT-2 board_update content-type evolution catalog verifier.
--
...
-- constraint definition; a mixed predecessor/target state; board_update
...
results AS (
  ...
  SELECT
    label || '_at_target_board_update_vocabulary',
    CASE WHEN definition_state = 'target' THEN 'PASS' ELSE 'FAIL' END,
    CASE
      WHEN definition_state = 'missing' THEN constraint_name || ' is missing'
      WHEN definition_state = 'target' THEN constraint_name || ' is at the exact P13-EXT-2 target vocabulary'
      WHEN definition_state = 'predecessor' THEN constraint_name || ' is still at the P13-EXT-1 predecessor vocabulary'
  ...
  SELECT
    ...
    'all three constraints must be at the same target board_update-inclusive state'
  FROM classified_state

  UNION ALL

  SELECT
    label || '_board_update_present_when_at_target',
    CASE
      WHEN definition_state = 'target' AND actual_def LIKE '%''board_update''::text%' THEN 'PASS'
      ELSE 'FAIL'
    END,
    constraint_name || ' includes board_update when reporting the target vocabulary'
  FROM classified_state
...
BEGIN
  IF EXISTS (
    WITH constraint_state(label, table_regclass, constraint_name, predecessor_def, target_def) AS (
      VALUES
        ('generation_runs', 'kai.generation_runs', 'generation_runs_p3_01_content_type_check', 'CHECK ((content_type = ANY (ARRAY[''evidence_summary''::text, ''impact_narrative''::text, ''readiness_assessment''::text, ''data_gap_memo''::text, ''case_for_support''::text])))', 'CHECK ((content_type = ANY (ARRAY[''evidence_summary''::text, ''impact_narrative''::text, ''readiness_assessment''::text, ''data_gap_memo''::text, ''case_for_support''::text, ''board_update''::text])))'),
    ...
    failures AS (
      ...
      SELECT 1
      FROM classified_state
      WHERE actual_def NOT LIKE '%''board_update''::text%'
      UNION ALL
      SELECT 1
    ...
    RAISE EXCEPTION 'P13-EXT-2 board_update content-type evolution verifier failed';
  END IF;
END $$;
