...
const dbName = "kai_p1_03_parser_profile_worker_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p1-03-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P1-03 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P1-03 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P1-03 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P1-03 runner refused non-loopback listen_addresses");
    if (row.version_num < 160000 || row.version_num >= 170000) throw new Error("P1-03 runner requires PostgreSQL 16");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  console.log(`P1-03 parser/profile worker ephemeral database created: ${dbName}`);
  console.log(`P1-03 parser/profile worker ephemeral PostgreSQL loopback: 127.0.0.1:${port}`);
  ...
  const testEnv = {
    ...
    DB_USER: user,
    DB_PASSWORD: "",
    KAI_P1_03_PARSER_WORKER_DATABASE_URL: targetUrl,
  };
  for (const spec of [
    ...
    if (testResult.status !== 0) throw new Error(`P1-03 parser/profile worker integration tests failed: ${spec}`);
  }
  console.log("P1-03 parser/profile worker integration tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P1-03 parser/profile worker ephemeral PostgreSQL workdir removed: ${workDir}`);
}
