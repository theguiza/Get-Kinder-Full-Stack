...
const dbName = "kai_c3_a3_provenance_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-c3-a3-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  if (!["127.0.0.1", "localhost", "::1"].includes(parsed.hostname.toLowerCase())) {
    throw new Error("C3.A3 provenance runner refused a non-loopback target before connecting");
  }
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("C3.A3 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`C3.A3 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("C3.A3 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("C3.A3 runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  // Full dependency chain this migration sits on top of (organization/
  // engagement anchors -> Gate A/P1 intake -> P2-01/P2-03 evidence/claims ->
  // A1.1-A1.4 -> B1.1 requirements -> C2.1 requirement_assessments -> P2-04
  // gap_log_items -> P2-05/P2-09/P2-10/P2-11/P2-12 review-decision ledgers),
  // exactly mirroring the existing C2.1 and P2-12 local-postgres runners.
  psqlFile("scripts/kai-sprint2-organization-enablement-bootstrap-synthetic-schema.sql");
  psqlExec(
    "ALTER TABLE kai.engagements ADD CONSTRAINT kai_c3_a3_engagements_id_org_unique UNIQUE (engagement_id, organization_id);",
  );
  ...
  // C3.A3 forward migration itself.
  ...
  if (focusedResult.status !== 0) throw new Error("C3.A3 provenance-extension tests failed");
  console.log("C3.A3 provenance-extension local PostgreSQL proof passed.");

  // Regression: the existing C2.1 requirement-assessment-persistence suite
  // must still pass unmodified against this same, now-extended schema -
  // proving the new tables/triggers do not disturb C2.1's own behavior.
  ...
  if (regressionResult.status !== 0) throw new Error("C2.1 regression suite failed against the C3.A3-extended schema");
  console.log("C2.1 regression suite passed against the C3.A3-extended schema.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`C3.A3 provenance-extension ephemeral PostgreSQL workdir removed: ${workDir}`);
}
