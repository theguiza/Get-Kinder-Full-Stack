# KAI P13-EXT-3 annual_report_section Content-Type Evolution Verification Runbook
...
bootstrap and prerequisite migrations through P13-EXT-2's `board_update`
content-type evolution already applied:

1. Apply `migrations/kai_sprint2_p13_ext3_annual_report_section_content_type_evolution.sql`.
2. Apply `migrations/kai_sprint2_p13_ext3_annual_report_section_export_candidate_content_type_evolution.sql`.
3. Run `scripts/kai-sprint2-p13-ext3-annual-report-section-content-type-evolution-verifier.sql`
   and confirm every row reports `PASS` (it raises otherwise).
4. Run `scripts/kai-sprint2-p13-ext3-annual-report-section-content-type-evolution-smoke-seed.sql`
   (synthetic data only).
5. Run `scripts/kai-sprint2-p13-ext3-annual-report-section-content-type-evolution-smoke-verifier.sql`
   and confirm every row reports `PASS`.
6. Run `scripts/kai-sprint2-p13-ext3-annual-report-section-content-type-evolution-failure-checks.sql`
   ...
   reports `PASS`.
7. Apply `migrations/kai_sprint2_p13_ext3_annual_report_section_export_candidate_content_type_evolution.rollback.sql`
   while the step-4 `annual_report_section` export-candidate row still
   exists, and confirm it is refused (data-loss guard).
8. Apply `migrations/kai_sprint2_p13_ext3_annual_report_section_content_type_evolution.rollback.sql`
   while the step-4 `annual_report_section` generation-run/draft rows still
   exist, and confirm it is refused (data-loss guard).
9. Remove the step-4 synthetic rows, then re-apply both rollback files and
   confirm they now succeed and restore the P13-EXT-2 vocabulary.
...
- any of the three constraints having an unrecognized/unexpected
  definition (neither the exact P13-EXT-2 predecessor definition nor the
  exact P13-EXT-3 target definition);
- a mixed state, where some of the three constraints are at the target
  `annual_report_section`-inclusive definition and others are still at the
  predecessor definition;
- `annual_report_section` being absent from any constraint that otherwise
...
- any content-type token appearing inside a constraint definition that is
  ...
  `case_for_support`, `board_update`, `annual_report_section`) - this
  guards against a future or unknown type (including `grant_response_paragraph`
  or `funder_outcome_table`) being silently admitted by a drifted or
  hand-edited constraint.
...
`board_update`, `annual_report_section`. No audience, status, queue, actor,
or export-manifest contract is touched, and `content_type` remains an
explicit `IN (...)` enumeration, never weakened to free text.
`grant_response_paragraph` and `funder_outcome_table` are not part of this
vocabulary and are not admitted by any constraint this package touches.

## Rollback

Both rollback files refuse (fail closed, `RAISE EXCEPTION`) when any
`annual_report_section` row exists in the table they would otherwise
