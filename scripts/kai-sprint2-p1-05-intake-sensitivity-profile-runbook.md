...
The runner also fails closed on any real `FAIL` status cell returned by the catalog verifier, read-only failure checks, or smoke verifier (`assertNoFail`, `scripts/kai-sprint2-p1-05-intake-sensitivity-profile-runner-assertions.js`), copying the established P1-02 `assertNoFail` repository pattern: a check name merely containing the substring `FAIL_CLOSED` never trips it. `__tests__/kai-sprint2-p1-05-intake-sensitivity-profile-runner-self-test.spec.js` proves this deterministically.
...
Every applicable classification dimension - PII, minor data, health/housing/justice/immigration data, Indigenous/OCAP-like governance-sensitive data (kept distinct from generic PII), staff notes, story/testimonial content, small-cell risk (kept distinct), consent basis, allowed use, and financial records (kept distinct from generic PII as its own Data Protection special category) - is its own column with its own CHECK-enforced `unknown` / `present` / `absent` (or `unknown` / `allowed` / `not_allowed`) enum. `unknown` is a real, distinct, queryable value: it never collapses into false/absent/clear/safe/permitted/not-applicable. No currently authorized profiler, validator, review service, or producer contract emits a classification, consent, sensitivity, or permission fact, so every dimension is pinned to `unknown` unconditionally by the column's own `DEFAULT 'unknown'`: this repository does not read `kai.intake_file_profiles.profile` at all (that column is machine-generated profiling metadata, never authoritative classification or consent input), and there is no `sensitivity_committed_facts`, `personal_data`, or other guessed producer shape recognized anywhere in this package. Loading only the tenant-scoped lineage facts required for P1-05 (`organization_id`, `intake_file_id`, `file_profile_id`, `profile_canonical_sha256`) is sufficient - a later, separately authorized package may introduce a real classification producer contract, but this foundation package does not guess at its shape.
