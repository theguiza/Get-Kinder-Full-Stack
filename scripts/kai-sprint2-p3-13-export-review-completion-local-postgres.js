...
const dbName = "kai_p3_13_export_review_completion_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p3-13-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  if (!["127.0.0.1", "localhost", "::1"].includes(parsed.hostname.toLowerCase())) {
    throw new Error("P3-13 runner refused non-loopback target before connection");
  }
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P3-13 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P3-13 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P3-13 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P3-13 runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  psqlExec(
    "ALTER TABLE kai.engagements ADD CONSTRAINT kai_p3_13_engagements_id_org_unique UNIQUE (engagement_id, organization_id);",
  );
  ...
  // export_review_p3_09_two_state_contract_removed check), so neither prior
  // verifier is re-run here once the P3-13 migration has been applied.
  ...
  // Real kai.organizations/kai.engagements rows the P3-13 (and cross-run
  // P3-09) integration suites' createEvidenceSummaryDraft/
  ...
  // each hardcode.
  psqlExec(
    "INSERT INTO kai.organizations (organization_id, name, organization_code) VALUES ('00000000-0000-4000-8000-000000000001', 'P3-13 Smoke Org', 'p3-13-smoke-org') ON CONFLICT (organization_id) DO NOTHING;",
  );
  psqlExec(
  ...
  if (testResult.status !== 0) throw new Error("P3-13 export-review-completion tests failed");
  console.log("P3-13 export-review-completion focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P3-13 export-review-completion ephemeral PostgreSQL workdir removed: ${workDir}`);
}
