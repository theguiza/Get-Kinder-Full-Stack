...
/**
 * P2-02 introduces no schema change: it reads only tables already committed
 * by the gate-a/P1/P2-01 migrations, applied here in the exact same order the
 * P2-01 runner already uses. No new migration, verifier.sql, failure-
 * checks.sql, or smoke-seed.sql is added by this package - the chained
 * P1/P2-01 smoke-seed scripts establish general schema availability, and the
 * package's own integration spec seeds every fixture its assertions actually
 * depend on.
 */
const repoRoot = new URL("..", import.meta.url).pathname.replace(/\/$/, "");
const dbName = "kai_p2_02_evidence_coverage_assessment_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p2-02-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P2-02 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P2-02 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P2-02 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P2-02 runner refused non-loopback listen_addresses");
    if (row.version_num < 160000 || row.version_num >= 170000) throw new Error("P2-02 runner requires PostgreSQL 16");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  console.log(`P2-02 evidence-coverage-assessment ephemeral database created: ${dbName}`);
  console.log(`P2-02 evidence-coverage-assessment ephemeral PostgreSQL loopback: 127.0.0.1:${port}`);
  ...
  const testEnv = {
    ...
    DB_USER: user,
    DB_PASSWORD: "",
    KAI_P2_02_EVIDENCE_COVERAGE_ASSESSMENT_DATABASE_URL: targetUrl,
  };
  for (const spec of [
    ...
    if (testResult.status !== 0) throw new Error(`P2-02 evidence-coverage-assessment integration tests failed: ${spec}`);
  }
  console.log("P2-02 evidence-coverage-assessment integration tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P2-02 evidence-coverage-assessment ephemeral PostgreSQL workdir removed: ${workDir}`);
}
