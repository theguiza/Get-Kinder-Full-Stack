...
const dbName = "kai_review_queue_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-review-queue-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("Review Queue runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`Review Queue runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("Review Queue runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("Review Queue runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  const testResult = spawnSync("node", [
    "--test",
    "__tests__/kai-sprint2-review-queue.integration.spec.js",
  ], {
    cwd: repoRoot,
    encoding: "utf8",
    stdio: "inherit",
    env: {
      ...
      DB_USER: user,
      DB_PASSWORD: "",
      KAI_REVIEW_QUEUE_DATABASE_URL: targetUrl,
    },
  });
  if (testResult.status !== 0) throw new Error("Review Queue tests failed");
  console.log("Review Queue focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`Review Queue ephemeral PostgreSQL workdir removed: ${workDir}`);
}
