...
BEGIN
  ...
  IF NOT ('waitlisted' = ANY (allowed_statuses)) THEN
    allowed_statuses := array_append(allowed_statuses, 'waitlisted');
  END IF;

  IF COALESCE(array_length(allowed_statuses, 1), 0) = 0 THEN
    allowed_statuses := ARRAY['pending', 'accepted', 'declined', 'checked_in', 'waitlisted'];
  END IF;
