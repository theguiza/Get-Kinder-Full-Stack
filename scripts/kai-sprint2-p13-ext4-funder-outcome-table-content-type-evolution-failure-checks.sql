-- KAI P13-EXT-4 funder_outcome_table content-type evolution failure checks.
--
-- Read-only. Proves from catalog definitions that unrelated content types,
-- near-miss funder_outcome_table-like tokens, and non-system generation-run
...
expected AS (
  SELECT
    ...
    'CHECK ((content_type = ANY (ARRAY[''evidence_summary''::text, ''impact_narrative''::text, ''readiness_assessment''::text, ''data_gap_memo''::text, ''case_for_support''::text, ''board_update''::text, ''annual_report_section''::text, ''funder_outcome_table''::text])))' AS expected_def
  UNION ALL
  SELECT
...
results AS (
  SELECT
    ...
    CASE
      ...
      THEN 'PASS' ELSE 'FAIL'
    END AS status,
    'generation_runs.content_type admits only the eight current application content types' AS detail
  FROM expected e
  ...
  SELECT
    'generation_runs_rejects_near_miss_funder_outcome_table_token',
    CASE
      WHEN c.constraint_def = e.expected_def
       AND c.constraint_def LIKE '%''funder_outcome_table''::text%'
       AND c.constraint_def NOT LIKE '%''funder_outcome_table_v2''::text%'
      THEN 'PASS' ELSE 'FAIL'
    END,
    'generation_runs.content_type remains an exact enumeration for funder_outcome_table'
  FROM expected e
  LEFT JOIN constraints c ON c.conname = e.conname
  WHERE e.conname = 'generation_runs_p3_01_content_type_check'
  ...
  SELECT
    ...
    CASE
      ...
      THEN 'PASS' ELSE 'FAIL'
    END,
    'P13-EXT-4 does not widen system-owned generation_run creation, even for funder_outcome_table'
  FROM expected e
  LEFT JOIN constraints c ON c.conname = e.conname
  ...
  SELECT
    ...
    CASE
      ...
      THEN 'PASS' ELSE 'FAIL'
    END,
    'export_candidates.content_type admits only the eight current application content types'
  FROM expected e
  LEFT JOIN constraints c ON c.conname = e.conname
  ...
  IF EXISTS (
    ...
    failures AS (
      ...
      WHERE conname = 'generation_runs_p3_01_content_type_check'
        AND constraint_def <> 'CHECK ((content_type = ANY (ARRAY[''evidence_summary''::text, ''impact_narrative''::text, ''readiness_assessment''::text, ''data_gap_memo''::text, ''case_for_support''::text, ''board_update''::text, ''annual_report_section''::text, ''funder_outcome_table''::text])))'
      UNION ALL
      SELECT 1
      FROM constraints
    ...
    RAISE EXCEPTION 'P13-EXT-4 funder_outcome_table content-type evolution failure-checks verifier failed';
  END IF;
END $$;
