...
const dbName = "kai_review_packet_contract_repair_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-rp-repair-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("review-packet-contract-repair runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`review-packet-contract-repair runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("review-packet-contract-repair runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("review-packet-contract-repair runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  run(psql, ["-v", "ON_ERROR_STOP=1", "-d", dbName, "-c", "ALTER TABLE kai.engagements ADD CONSTRAINT kai_review_packet_contract_repair_engagements_id_org_unique UNIQUE (engagement_id, organization_id);"], { capture: true });
  ...
  const testResult = spawnSync("node", [
    ...
    env: {
      ...
      DB_USER: user,
      DB_PASSWORD: "",
      KAI_REVIEW_PACKET_CONTRACT_REPAIR_DATABASE_URL: targetUrl,
  ...
  if (testResult.status !== 0) throw new Error("review-packet-traceability-contract-repair tests failed");
  console.log("review-packet-traceability-contract-repair focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`review-packet-traceability-contract-repair ephemeral PostgreSQL workdir removed: ${workDir}`);
}
