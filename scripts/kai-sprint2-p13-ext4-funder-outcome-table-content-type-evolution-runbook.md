# KAI P13-EXT-4 funder_outcome_table Content-Type Evolution Verification Runbook
...
bootstrap and prerequisite migrations through P13-EXT-3's
`annual_report_section` content-type evolution already applied:

1. Apply `migrations/kai_sprint2_p13_ext4_funder_outcome_table_content_type_evolution.sql`.
2. Apply `migrations/kai_sprint2_p13_ext4_funder_outcome_table_export_candidate_content_type_evolution.sql`.
3. Run `scripts/kai-sprint2-p13-ext4-funder-outcome-table-content-type-evolution-verifier.sql`
   and confirm every row reports `PASS` (it raises otherwise).
4. Run `scripts/kai-sprint2-p13-ext4-funder-outcome-table-content-type-evolution-smoke-seed.sql`
   (synthetic data only).
5. Run `scripts/kai-sprint2-p13-ext4-funder-outcome-table-content-type-evolution-smoke-verifier.sql`
   and confirm every row reports `PASS`.
6. Run `scripts/kai-sprint2-p13-ext4-funder-outcome-table-content-type-evolution-failure-checks.sql`
   ...
   reports `PASS`.
7. Apply `migrations/kai_sprint2_p13_ext4_funder_outcome_table_export_candidate_content_type_evolution.rollback.sql`
   while the step-4 `funder_outcome_table` export-candidate row still
   exists, and confirm it is refused (data-loss guard).
8. Apply `migrations/kai_sprint2_p13_ext4_funder_outcome_table_content_type_evolution.rollback.sql`
   while the step-4 `funder_outcome_table` generation-run/draft rows still
   exist, and confirm it is refused (data-loss guard).
9. Remove the step-4 synthetic rows, then re-apply both rollback files and
   confirm they now succeed and restore the P13-EXT-3 vocabulary.
...
- any of the three constraints having an unrecognized/unexpected
  definition (neither the exact P13-EXT-3 predecessor definition nor the
  exact P13-EXT-4 target definition);
- a mixed state, where some of the three constraints are at the target
  `funder_outcome_table`-inclusive definition and others are still at the
  predecessor definition;
- `funder_outcome_table` being absent from any constraint that otherwise
  reports itself as being at the target definition;
- any content-type token appearing inside a constraint definition that is
  ...
  `funder_outcome_table`) - this guards against a future or unknown type
  (including `grant_response_paragraph`) being silently admitted by a
  drifted or hand-edited constraint.
...
`board_update`, `annual_report_section`, `funder_outcome_table`. No
audience, status, queue, actor, or export-manifest contract is touched, and
`content_type` remains an explicit `IN (...)` enumeration, never weakened
to free text. `grant_response_paragraph` is not part of this vocabulary and
is not admitted by any constraint this package touches.

## Rollback

Both rollback files refuse (fail closed, `RAISE EXCEPTION`) when any
`funder_outcome_table` row exists in the table they would otherwise narrow,
rather than deleting or rewriting `content_type` values, following the same
guard shape used by every prior extension's rollback.
