-- KAI P13-EXT-4 funder_outcome_table content-type evolution catalog verifier.
--
...
-- constraint definition; a mixed predecessor/target state; funder_outcome_table
...
results AS (
  ...
  SELECT
    label || '_at_target_funder_outcome_table_vocabulary',
    CASE WHEN definition_state = 'target' THEN 'PASS' ELSE 'FAIL' END,
    CASE
      WHEN definition_state = 'missing' THEN constraint_name || ' is missing'
      WHEN definition_state = 'target' THEN constraint_name || ' is at the exact P13-EXT-4 target vocabulary'
      WHEN definition_state = 'predecessor' THEN constraint_name || ' is still at the P13-EXT-3 predecessor vocabulary'
  ...
  SELECT
    ...
    'all three constraints must be at the same target funder_outcome_table-inclusive state'
  FROM classified_state

  UNION ALL

  SELECT
    label || '_funder_outcome_table_present_when_at_target',
    CASE
      WHEN definition_state = 'target' AND actual_def LIKE '%''funder_outcome_table''::text%' THEN 'PASS'
      ELSE 'FAIL'
    END,
    constraint_name || ' includes funder_outcome_table when reporting the target vocabulary'
  FROM classified_state
    ...
    CASE
      ...
      WHEN EXISTS (
        ...
        WHERE m.token[1] NOT IN (
          ...
          'annual_report_section', 'funder_outcome_table'
        )
      ) THEN 'FAIL'
      ELSE 'PASS'
    END,
...
BEGIN
  IF EXISTS (
    WITH constraint_state(label, table_regclass, constraint_name, predecessor_def, target_def) AS (
      VALUES
        ('generation_runs', 'kai.generation_runs', 'generation_runs_p3_01_content_type_check', 'CHECK ((content_type = ANY (ARRAY[''evidence_summary''::text, ''impact_narrative''::text, ''readiness_assessment''::text, ''data_gap_memo''::text, ''case_for_support''::text, ''board_update''::text, ''annual_report_section''::text])))', 'CHECK ((content_type = ANY (ARRAY[''evidence_summary''::text, ''impact_narrative''::text, ''readiness_assessment''::text, ''data_gap_memo''::text, ''case_for_support''::text, ''board_update''::text, ''annual_report_section''::text, ''funder_outcome_table''::text])))'),
    ...
    failures AS (
      ...
      SELECT 1
      FROM classified_state
      WHERE actual_def NOT LIKE '%''funder_outcome_table''::text%'
      UNION ALL
      SELECT 1
    ...
    RAISE EXCEPTION 'P13-EXT-4 funder_outcome_table content-type evolution verifier failed';
  END IF;
END $$;
