...
const dbName = "kai_c3b2_c_concurrent_currentness_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-c3b2-c-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  if (!["127.0.0.1", "localhost", "::1"].includes(parsed.hostname.toLowerCase())) {
    throw new Error("C3.B2.C concurrent-currentness runner refused a non-loopback target before connecting");
  }
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("C3.B2.C runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`C3.B2.C runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("C3.B2.C runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("C3.B2.C runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  // migration guard requires to exist.
  psqlFile("scripts/kai-sprint2-organization-enablement-bootstrap-synthetic-schema.sql");
  psqlExec(
    "ALTER TABLE kai.engagements ADD CONSTRAINT kai_c3b2_c_engagements_id_org_unique UNIQUE (engagement_id, organization_id);",
  );

  // Full Gate A / P1 / P2-01..P2-12 / A1.1-A1.4 / B1.1 / C2.1 / C3.A3 chain -
  // both ir_contrib_002 and ir_comm_002 share this exact schema chain; no
  // new migration is introduced by this proof.
  ...
  if (testResult.status !== 0) throw new Error("C3.B2.C concurrent-currentness integration tests failed");
  console.log("C3.B2.C concurrent-currentness local PostgreSQL proof passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`C3.B2.C concurrent-currentness ephemeral PostgreSQL workdir removed: ${workDir}`);
}
