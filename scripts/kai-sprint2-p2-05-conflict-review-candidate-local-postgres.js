...
import { assertNoFail } from "./kai-sprint2-p2-05-conflict-review-candidate-runner-assertions.js";

const repoRoot = new URL("..", import.meta.url).pathname.replace(/\/$/, "");
const dbName = "kai_p2_05_conflict_review_candidate_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p2-05-pg-"));
const dataDir = join(workDir, "data");
const socketDir = join(workDir, "socket");
const logFile = join(workDir, "postgres.log");
const port = String(60000 + Math.floor(Math.random() * 1000));
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P2-05 runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P2-05 runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P2-05 runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P2-05 runner refused non-loopback listen_addresses");
    if (row.version_num < 160000 || row.version_num >= 170000) throw new Error("P2-05 runner requires PostgreSQL 16");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  console.log(`P2-05 conflict-review-candidate ephemeral database created: ${dbName}`);
  console.log(`P2-05 conflict-review-candidate ephemeral PostgreSQL loopback: 127.0.0.1:${port}`);
  ...
  const catalogOutput = psqlFile("scripts/kai-sprint2-p2-05-conflict-review-candidate-verifier.sql");
  console.log(catalogOutput);
  assertNoFail("P2-05 catalog verifier", catalogOutput);

  const failureOutput = psqlFile("scripts/kai-sprint2-p2-05-conflict-review-candidate-failure-checks.sql");
  console.log(failureOutput);
  assertNoFail("P2-05 failure checks", failureOutput);
  ...
  const testEnv = {
    ...
    DB_USER: user,
    DB_PASSWORD: "",
    KAI_P2_05_CONFLICT_REVIEW_CANDIDATE_DATABASE_URL: targetUrl,
  };
  ...
  if (testResult.status !== 0) {
    throw new Error("P2-05 conflict-review-candidate boundary tests failed");
  }
  console.log("P2-05 conflict-review-candidate focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P2-05 conflict-review-candidate ephemeral PostgreSQL workdir removed: ${workDir}`);
}
