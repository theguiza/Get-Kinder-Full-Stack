...
const dbName = "kai_p14_11_generated_content_draft_audit_composition_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p14-11-gcac-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P14-11 generated-content-draft-audit-composition runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P14-11 generated-content-draft-audit-composition runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P14-11 generated-content-draft-audit-composition runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P14-11 generated-content-draft-audit-composition runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  run(psql, ["-v", "ON_ERROR_STOP=1", "-d", dbName, "-c", "ALTER TABLE kai.engagements ADD CONSTRAINT kai_p14_11_gcac_engagements_id_org_unique UNIQUE (engagement_id, organization_id);"], { capture: true });
  ...
  const testResult = spawnSync("node", [
    "--test",
    "__tests__/kai-sprint2-p14-11-generated-content-draft-audit-composition-repair.integration.spec.js",
  ], {
    cwd: repoRoot,
    encoding: "utf8",
    stdio: "inherit",
    env: {
      ...
      DB_USER: user,
      DB_PASSWORD: "",
      KAI_P14_11_GENERATED_CONTENT_AUDIT_DATABASE_URL: targetUrl,
    },
  });
  if (testResult.status !== 0) throw new Error("P14-11 generated-content-draft-audit-composition-repair tests failed");
  console.log("P14-11 generated-content-draft-audit-composition-repair focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P14-11 generated-content-draft-audit-composition-repair ephemeral PostgreSQL workdir removed: ${workDir}`);
}
