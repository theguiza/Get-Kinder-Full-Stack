...
const dbName = "kai_p14_12_funder_evidence_http_route_synthetic";
...
const workDir = mkdtempSync(join(tmpdir(), "kai-p14-12-fehr-pg-"));
...
async function proveRunnerOwnedTarget() {
  ...
  try {
    ...
    if (row.database_name !== dbName) throw new Error("P14-12 funder-evidence-http-route runner refused non-synthetic database name");
    if (!["127.0.0.1", "127.0.0.1/32", "::1", "::ffff:127.0.0.1"].includes(row.server_addr)) {
      throw new Error(`P14-12 funder-evidence-http-route runner refused non-loopback server address: ${row.server_addr}`);
    }
    if (row.server_port !== port) throw new Error("P14-12 funder-evidence-http-route runner refused unexpected PostgreSQL port");
    if (row.listen_addresses !== "127.0.0.1") throw new Error("P14-12 funder-evidence-http-route runner refused non-loopback listen_addresses");
  } finally {
    await client.end();
  }
}
...
try {
  ...
  run(psql, ["-v", "ON_ERROR_STOP=1", "-d", dbName, "-c", "ALTER TABLE kai.engagements ADD CONSTRAINT kai_p14_12_fehr_engagements_id_org_unique UNIQUE (engagement_id, organization_id);"], { capture: true });
  ...
  const testResult = spawnSync("node", [
    "--test",
    "__tests__/kai-sprint2-p14-12-funder-evidence-summary-http-route.integration.spec.js",
  ], {
    cwd: repoRoot,
    encoding: "utf8",
    stdio: "inherit",
    env: {
      ...process.env,
      // Backend/db/pg.js hardcodes ssl:{rejectUnauthorized:false} on every
      // connection-string branch (DATABASE_URL_LOCAL included), which a
      // plain loopback ephemeral PostgreSQL (no SSL) refuses outright. Its
      // own designed no-connection-string fallback (DB_HOST/DB_PORT/
      // DB_NAME/DB_USER/DB_PASSWORD, no ssl option) is the only path that
      // reaches a real local instance, so every connection-string
      // candidate is cleared here and that fallback is used instead - this
      // is this run's explicitly authorized real-database target, still
      // strictly loopback/synthetic-only.
      NODE_ENV: "",
      DATABASE_URL: "",
      DATABASE_URL_LOCAL: "",
      PGURL_LOCAL: "",
      ...
      DB_USER: user,
      DB_PASSWORD: "",
      KAI_P14_12_FUNDER_EVIDENCE_HTTP_DATABASE_URL: targetUrl,
      KAI_SPRINT2_ENABLED: "true",
      KAI_GENERATION_ENABLED: "true",
      ANTHROPIC_API_KEY: process.env.ANTHROPIC_API_KEY || "sk-ant-test-unused-dummy-key",
    },
  });
  if (testResult.status !== 0) throw new Error("P14-12 funder-evidence-summary-http-route tests failed");
  console.log("P14-12 funder-evidence-summary-http-route focused tests passed.");
} finally {
  if (started) spawnSync(pgCtl, ["-D", dataDir, "stop", "-m", "fast"], { encoding: "utf8", stdio: "ignore" });
  rmSync(workDir, { recursive: true, force: true });
  console.log(`P14-12 funder-evidence-summary-http-route ephemeral PostgreSQL workdir removed: ${workDir}`);
}
